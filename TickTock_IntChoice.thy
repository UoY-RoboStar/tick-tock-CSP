...
definition IntChoiceTTT :: "'e cttobs list set \<Rightarrow> 'e cttobs list set \<Rightarrow> 'e cttobs list set" (infixl "\<sqinter>\<^sub>C" 56) where
  "P \<sqinter>\<^sub>C Q = P \<union> Q"

lemma IntChoiceTTT_wf: "\<forall> t\<in>P. ttWF t \<Longrightarrow> \<forall> t\<in>Q. ttWF t \<Longrightarrow> \<forall> t\<in>P \<sqinter>\<^sub>C Q. ttWF t"
  unfolding IntChoiceTTT_def by auto

lemma TT2s_IntChoice:
  assumes "TT2s P" "TT2s Q"
  shows "TT2s (P \<sqinter>\<^sub>C Q)"
    using assms unfolding IntChoiceTTT_def TT2s_def by (auto, (smt disjoint_iff_not_equal mem_Collect_eq)+)

lemma TT4s_IntChoice:
  assumes "TT4s P" "TT4s Q"
  shows "TT4s (P \<sqinter>\<^sub>C Q)"
  using assms unfolding IntChoiceTTT_def TT4s_def by auto
...
proof auto
  ...
  show "x \<in> P \<sqinter>\<^sub>C Q \<Longrightarrow> ttWF x"
    by (metis TT_wf IntChoiceTTT_def Un_iff assms(1) assms(2))
next
  show "P \<sqinter>\<^sub>C Q = {} \<Longrightarrow> False"
    using TT_empty IntChoiceTTT_def assms(1) by blast
next
  fix \<rho> \<sigma>
  show "\<rho> \<lesssim>\<^sub>C \<sigma> \<Longrightarrow> \<sigma> \<in> P \<sqinter>\<^sub>C Q \<Longrightarrow> \<rho> \<in> P \<sqinter>\<^sub>C Q"
    by (metis TT1_def TT_TT1 IntChoiceTTT_def Un_iff assms(1) assms(2))
next
  fix \<rho> X Y
  ...
  have 1: "\<rho> @ [[X]\<^sub>R] \<in> P \<or> \<rho> @ [[X]\<^sub>R] \<in> Q"
    using assm1 unfolding IntChoiceTTT_def by auto
  ...
  show "\<rho> @ [[X \<union> Y]\<^sub>R] \<in> P \<sqinter>\<^sub>C Q"
    using "1" "5" "6" IntChoiceTTT_def by blast
next
  fix x
  show "x \<in> P \<sqinter>\<^sub>C Q \<Longrightarrow> TT3_trace x"
    by (metis TT3_def TT_TT3 IntChoiceTTT_def UnE assms(1) assms(2))
qed 
