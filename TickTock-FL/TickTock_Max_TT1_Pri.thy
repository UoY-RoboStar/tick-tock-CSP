...
proof -
  ...
  (*also have "... =
    ...
    using assms 
    using TT3ww_fixpoint_mkTT3ww mkTT3ww_def apply fastforce*)
  finally show ?thesis .
qed
...
lemma TTMPick_extend_Ref:
  assumes "TTMPick zs (s @ [[X]\<^sub>R]) P" "TT3w P" "TT2 P" "TT1 P"
...
next
  ...
  next
    ...
    then have "\<forall>e. e \<noteq> Tock \<and> e \<notin> x2 \<longrightarrow> s @ [[X \<union> {e. e \<noteq> Tock \<and> s @ [[e]\<^sub>E] \<notin> P \<or> e = Tock \<and> s @ [[X]\<^sub>R, [Tock]\<^sub>E] \<notin> P} \<union> {Tick}]\<^sub>R] @ xs @ [[e]\<^sub>E] \<in> P"
      using assms TT2_TT3w_extends_Ref by blast
...
next
  ...
  then show ?case
  proof (auto)
    assume "TT1 P" "TT3w P" "TT2 P" "s @ [[X]\<^sub>R, [Tock]\<^sub>E] \<in> P"
  ...
  next
    assume healths:"TT1 P" "TT3w P" "TT2 P" "TTMPick (mkTTMP xs (s @ [[X]\<^sub>R]) P) (s @ [[X]\<^sub>R]) P"
...
lemma priMaxTT_start_Ref_extends:
  assumes "TT1 P" "TT2 P" "TT3w P" "priMaxTT pa t s (sa @ zs) (unTT1 Q)"
...
lemma mkTTMP_in_P:
  assumes "s @ z \<in> P" "TT3w P" "ttWFx P" "TT2 P" "TT1 P"
...
next
  ...
  next
    ...
    then have "y @ [[x2 \<union> {e. (e \<noteq> Tock \<and> xs @ [[e]\<^sub>E] \<notin> P) \<or> (e = Tock \<and> xs @ [[x2]\<^sub>R,[Tock]\<^sub>E] \<notin> P)} \<union> {Tick}]\<^sub>R] @ z \<in> P"
      using TT3w_def by (meson TT3w_middle_Ref_with_Tick snoc.prems(2) snoc.prems(5))
...
proof -
  ...
  also have "... =
    ...
    using assms TTM3_mkTT1_simp TT3w_TT1_imp_TT3ww by auto
  also have "... =
...
lemma concat_unTT1_extend_TT2_Refusal:
  assumes "xs @ [[Sa]\<^sub>R] @ ys \<in> unTT1 Q" "TT2 Q" "TT1 Q" "TT3ww Q"
...
lemma priMaxTT_start_Ref_extends:
  assumes "TT1 Q" "TT2 Q" "TT3ww Q" "priMaxTT pa t s (sa @ [[S]\<^sub>R, [Tock]\<^sub>E] @ z) (unTT1 Q)" 
  ...
  proof -
    ...
    then have "saa @ [[Sa \<union> {e. e \<noteq> Tock \<and> saa @ [[e]\<^sub>E] \<notin> Qa \<or> e = Tock \<and> saa @ [[Sa]\<^sub>R, [Tock]\<^sub>E] \<notin> Qa} \<union> {Tick}]\<^sub>R] @ [Tock]\<^sub>E # sb @ [[Z]\<^sub>R] \<in> unTT1 Qa"
      using TT1_healthy TT2_healthy TT3ww_healthy concat_unTT1_extend_TT2_Refusal by blast
    then have "saa @ [Sa \<union> {e. e \<noteq> Tock \<and> saa @ [[e]\<^sub>E] \<notin> Qa \<or> e = Tock \<and> saa @ [[Sa]\<^sub>R, [Tock]\<^sub>E] \<notin> Qa} \<union> {Tick}]\<^sub>R # [Tock]\<^sub>E # sb @ [[Z]\<^sub>R] \<in> unTT1 Qa"
      by auto
...
lemma priTT1_TTMPick_imp_priMaxTT:
  assumes "priTT1 p x s i P" "TT3w P" "ttWFx P" "TT2 P" "TT1 P"
...
proof -
  ...
  next
    ...
    then have TT3ww_healthy: "TT3ww Q" 
      using TT3w_healthy TT1_healthy TT3w_TT1_imp_TT3ww by blast
    ...
    proof -
      ...
      proof -
        ...
        then have "priMaxTT pa t Y ((mkTTMP sa [] Q) @ [[S \<union> {e. e \<noteq> Tock \<and> sa @ [[e]\<^sub>E] \<notin> Q \<or> e = Tock \<and> sa @ [[S]\<^sub>R, [Tock]\<^sub>E] \<notin> Q} \<union> {Tick}]\<^sub>R, [Tock]\<^sub>E]) (unTT1 Q)"
          (*using TT1_healthy TT2_healthy TT3ww_healthy Y Z priMaxTT_start_Ref_extends sledgehammer by fastforce*)
          using mkTTMP_absorb_Ref_Tock'
...
(*    and TTMPick_sa:   "TTMPick sa [] Q"*)
   ...
   proof -
      ...
      have TT3ww_healthy:"TT3ww Q"
        using TT1_healthy TT3w_healthy 
        by (simp add: TT3w_TT1_imp_TT3ww)
      ...
      then have "(mkTTMP sa [] Q) \<in> Q"
        by (meson TT1_def TT1_healthy TT2_healthy ttWFx_healthy TT3w_healthy TTs_mkTTMP_in_P tt_prefix_subset_front tt_prefix_subset_refl)
      then have b:"(mkTTMP sa [] Q) @ [[Z]\<^sub>R] \<in> unTT1 Q \<and> e\<^sub>2 \<notin> Z \<and> \<not>(\<exists>b. b \<notin> Z \<and> e\<^sub>2 <\<^sup>*pa b)"
        using a TT1_healthy TT3ww_healthy  
        by (simp add: priMaxTT_unTT1_case_specific TT2_healthy ttWFx_healthy TT3w_healthy mkTTMP_in_P)
...
lemma priMaxTT_imp_priTT1:
  assumes "x \<lesssim>\<^sub>C t" "TTMPick s i P" "priMaxTT p t s i (unTT1 P)" "TT3w P" "ttWFx P" "TT2 P" "TT1 P"
      ...
      proof (cases a)
        ...
        proof -
          ...
          then have "(sa @ [[Z]\<^sub>R] \<in> Q \<and> TTMPick sa [] Q \<and> e\<^sub>2 \<notin> Z \<and> \<not>(\<exists>b. b \<notin> Z \<and> e\<^sub>2 <\<^sup>*pa b)
                      \<and> (\<forall>e. e \<notin> Z \<and> e \<noteq> Tock \<longrightarrow> sa @ [[e]\<^sub>E] \<in> Q)
                      \<and> (Tock \<notin> Z \<longrightarrow> sa @ [[Z]\<^sub>R,[Tock]\<^sub>E] \<in> Q) \<and> Tick \<in> Z)"
            using TT1_healthy TT3ww_healthy priMaxTT_unTT1_case by blast
...
lemma mkTT1_PriMax_unTT1_priTT:
  assumes "TT1 P" "TT3ww P" "TT3w P" "ttWFx P" "TT2 P"
