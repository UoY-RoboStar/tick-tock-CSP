...
fun mkTTick_Trace :: "'e cttobs list \<Rightarrow> 'e cttobs list" where
"mkTTick_Trace [] = []" |
...
next
  ...
  next
    ...
    have a:"Tick \<in> x2"
      using snoc Ref
      by (metis TTickAll_def TTickTrace.elims(2) TTickTrace_dist_concat cttobs.distinct(1) cttobs.inject(2) list.inject not_Cons_self2)
