...
function cttWF2 :: "'e cttobs list \<Rightarrow> 'e cttobs list \<Rightarrow> bool" where
  ...
  by (simp_all, case_tac x, simp, case_tac a rule:cttWF.cases, (case_tac b rule:cttWF.cases, simp_all)+)
...
datatype 'e eventtockref = TEvent "'e" ("[_]`\<^sub>E") | TRef "'e cttevent set" ("[_]`\<^sub>T") | TTock
print_theorems
datatype 'e endevent = Instability ("\<bullet>") | ERef "'e cttevent set" ("[_]`\<^sub>R") | ETick
print_theorems
datatype 'e ctt = CTT "'e eventtockref list" "'e endevent"
print_theorems


(* no need for wellformedness conditions *)

lemma ctt_induct:
  "(P (CTT [] \<bullet>)) \<Longrightarrow>
    (P (CTT [] ETick)) \<Longrightarrow>
    (\<And> X. P (CTT [] [X]`\<^sub>R)) \<Longrightarrow>
    (\<And> xa. P (CTT xa \<bullet>) \<longrightarrow> P (CTT (TTock # xa) \<bullet>)) \<Longrightarrow>
    (\<And> xa. P (CTT xa ETick) \<longrightarrow> P (CTT (TTock # xa) ETick)) \<Longrightarrow>
    (\<And> xa X. P (CTT xa [X]`\<^sub>R) \<longrightarrow> P (CTT (TTock # xa) [X]`\<^sub>R)) \<Longrightarrow>
    (\<And> xa e. P (CTT xa \<bullet>) \<longrightarrow> P (CTT ([e]`\<^sub>E # xa) \<bullet>)) \<Longrightarrow>
    (\<And> xa e. P (CTT xa ETick) \<longrightarrow> P (CTT ([e]`\<^sub>E # xa) ETick)) \<Longrightarrow>
    (\<And> xa X e. P (CTT xa [X]`\<^sub>R) \<longrightarrow> P (CTT ([e]`\<^sub>E # xa) [X]`\<^sub>R))\<Longrightarrow>
    (\<And> xa Y. P (CTT xa \<bullet>) \<longrightarrow> P (CTT ([Y]`\<^sub>T # xa) \<bullet>)) \<Longrightarrow>
    (\<And> xa Y. P (CTT xa ETick) \<longrightarrow> P (CTT ([Y]`\<^sub>T # xa) ETick)) \<Longrightarrow>
    (\<And> xa X Y. P (CTT xa [X]`\<^sub>R) \<longrightarrow> P (CTT ([Y]`\<^sub>T # xa) [X]`\<^sub>R)) \<Longrightarrow> P x"
  apply (induct x)
  apply (induct_tac x1a, auto, induct_tac x2a, auto)
  apply (induct_tac a, auto, (case_tac x2a, auto)+)
  done

lemma ctt_cases: "(x = (CTT [] \<bullet>)) \<or>
    (x = (CTT [] ETick)) \<or>
    (\<exists> X. x = (CTT [] [X]`\<^sub>R)) \<or>
    (\<exists> xa. x = (CTT (TTock # xa) \<bullet>)) \<or>
    (\<exists> xa. x = (CTT (TTock # xa) ETick)) \<or>
    (\<exists> xa X. x = (CTT (TTock # xa) [X]`\<^sub>R)) \<or>
    (\<exists> xa e. x = (CTT ([e]`\<^sub>E # xa) \<bullet>)) \<or>
    (\<exists> xa e. x = (CTT ([e]`\<^sub>E # xa) ETick)) \<or>
    (\<exists> xa X e. x = (CTT ([e]`\<^sub>E # xa) [X]`\<^sub>R))\<or>
    (\<exists> xa Y. x = (CTT ([Y]`\<^sub>T # xa) \<bullet>)) \<or>
    (\<exists> xa Y. x = (CTT ([Y]`\<^sub>T # xa) ETick)) \<or>
    (\<exists> xa X Y. x = (CTT ([Y]`\<^sub>T # xa) [X]`\<^sub>R))"
  apply (cases x, simp)
  apply (case_tac x1, simp_all, case_tac x2, simp_all)
  apply (case_tac a, simp_all, (case_tac x2, auto)+)
  done

fun eventtockref_order :: "'e eventtockref \<Rightarrow> 'e eventtockref \<Rightarrow> bool" (infix "\<subseteq>\<^sub>T" 50) where
  "eventtockref_order TTock TTock = True" |
  "eventtockref_order TTock [Y]`\<^sub>T = True" |
  "eventtockref_order [X]`\<^sub>T [Y]`\<^sub>T = (X \<subseteq> Y)" |
  "eventtockref_order [x]`\<^sub>E [y]`\<^sub>E = (x = y)" |
  "eventtockref_order x y = False"

lemma eventtockref_order_refl: "x \<subseteq>\<^sub>T x"
  using eventtockref_order.elims(3) by blast

lemma eventtockref_order_trans: "x \<subseteq>\<^sub>T y \<Longrightarrow> y \<subseteq>\<^sub>T z \<Longrightarrow> x \<subseteq>\<^sub>T z"
  by (induct rule:eventtockref_order.induct, auto, (cases y, auto)+)

lemma eventtockref_order_antisym: "x \<subseteq>\<^sub>T y \<Longrightarrow> y \<subseteq>\<^sub>T x \<Longrightarrow> x = y"
  by (induct rule:eventtockref_order.induct, auto)

fun ctt_prefix' :: "'e ctt \<Rightarrow> 'e ctt \<Rightarrow> bool" (infix "\<le>`\<^sub>C" 50) where
  "ctt_prefix' (CTT [] \<bullet>) x = True" |
  "ctt_prefix' (CTT [] x) (CTT [] y) = (x = y)" |
  "ctt_prefix' (CTT (x # xa) xb) (CTT (y # ya) yb) = (x = y \<and> ctt_prefix' (CTT xa xb) (CTT ya yb))" |
  "ctt_prefix' x y = False"

fun ctt_prefix_subset' :: "'e ctt \<Rightarrow> 'e ctt \<Rightarrow> bool" (infix "\<lesssim>`\<^sub>C" 50) where
  "ctt_prefix_subset' (CTT [] \<bullet>) x = True" |
  "ctt_prefix_subset' (CTT [] ETick) (CTT [] ETick) = True" |
  "ctt_prefix_subset' (CTT [] [X]`\<^sub>R) (CTT [] [Y]`\<^sub>R) = (X \<subseteq> Y)" |
  "ctt_prefix_subset' (CTT (x # xa) xb) (CTT (y # ya) yb) = (x \<subseteq>\<^sub>T y \<and> ctt_prefix_subset' (CTT xa xb) (CTT ya yb))" |
  "ctt_prefix_subset' x y = False"

lemma ctt_prefix'_refl: "x \<le>`\<^sub>C x"
  by (induct x rule:ctt_induct, auto)

lemma ctt_prefix'_trans: "x \<le>`\<^sub>C y \<Longrightarrow> y \<le>`\<^sub>C z \<Longrightarrow> x \<le>`\<^sub>C z"
proof -
  have "\<exists>y. x \<le>`\<^sub>C y \<and> y \<le>`\<^sub>C z \<Longrightarrow> x \<le>`\<^sub>C z"
  proof (induct rule:ctt_prefix'.induct, auto)
    fix v :: "'e cttevent set"
    fix y ya
    assume assm1: "CTT [] [v]`\<^sub>R \<le>`\<^sub>C ya"
    assume assm2: "ya \<le>`\<^sub>C CTT [] y"
    obtain va where "ya = CTT [] [va]`\<^sub>R"
      using assm1 ctt_prefix'.elims(2) by auto
    then show "[v]`\<^sub>R = y"
      using assm1 assm2 by auto
  next
    fix y ya
    assume assm1: "CTT [] ETick \<le>`\<^sub>C ya"
    assume assm2: "ya \<le>`\<^sub>C CTT [] y"
    have "ya = CTT [] ETick"
      using assm1 ctt_prefix'.elims(2) by auto
    then show "ETick = y"
      using assm1 assm2 by auto
  next
    fix x xa xb y ya yb yc
    assume assm1: "CTT (x # xa) xb \<le>`\<^sub>C yc"
    assume assm2: "yc \<le>`\<^sub>C CTT (y # ya) yb"
    obtain xa' xb' where "yc = CTT (x # xa') xb'"
      by (metis assm1 ctt.exhaust ctt_prefix'.simps(4) ctt_prefix'.simps(5) list.exhaust)
    then show "x = y"
      using assm2 by auto
  next
    fix x xa xb y ya yb yc
    assume assm1: "\<exists>y. CTT xa xb \<le>`\<^sub>C y \<and> y \<le>`\<^sub>C CTT ya yb \<Longrightarrow> CTT xa xb \<le>`\<^sub>C CTT ya yb"
    assume assm2: "CTT (x # xa) xb \<le>`\<^sub>C yc"
    assume assm3: "yc \<le>`\<^sub>C CTT (y # ya) yb"
    obtain yca ycb ycc where "yc = CTT (yca # ycb) ycc"
      by (metis assm2 ctt.exhaust ctt_prefix'.simps(5) list.exhaust)
    then show "CTT xa xb \<le>`\<^sub>C CTT ya yb"
      using assm1 assm2 assm3 ctt_prefix'.simps(4) by blast
  next
    fix vb vc va vd y
    assume assm1: "CTT (vb # vc) va \<le>`\<^sub>C y"
    assume assm2: "y \<le>`\<^sub>C CTT [] vd"
    obtain ya where "y = CTT [] ya"
      using assm2 ctt_prefix'.elims(2) by blast
    then show "False"
      using assm1 by auto
  next
    fix vb vc va vd y
  ...
  next
    fix vb vd ve vc y
    assume assm1: "CTT [] [vb]`\<^sub>R \<le>`\<^sub>C y"
    assume assm2: "y \<le>`\<^sub>C CTT (vd # ve) vc"
    obtain ya yb yc where "y = CTT (ya # yb) yc"
      by (metis assm1 assm2 ctt.exhaust ctt_prefix'.simps(2) ctt_prefix'.simps(7) list.exhaust)
    then show "False"
      using assm1 by auto
  next
      fix vb vd vc y
    assume assm1: "CTT [] ETick \<le>`\<^sub>C y"
    assume assm2: "y \<le>`\<^sub>C CTT (vc # vd) vb"
    obtain ya yb yc where "y = CTT (ya # yb) yc"
      by (metis assm1 assm2 ctt.exhaust ctt_prefix'.simps(3) ctt_prefix'.simps(9) list.exhaust)
    then show "False"
      using assm1 by auto
  qed
  then show "x \<le>`\<^sub>C y \<Longrightarrow> y \<le>`\<^sub>C z \<Longrightarrow> x \<le>`\<^sub>C z" by auto
...
lemma ctt_prefix'_antisym: "x \<le>`\<^sub>C y \<Longrightarrow> y \<le>`\<^sub>C x \<Longrightarrow> x = y"
  using ctt_prefix'.elims(2) by (induct rule:ctt_prefix'.induct, auto)

lemma ctt_prefix_subset'_refl: "x \<lesssim>`\<^sub>C x"
  by (induct rule:ctt_induct, auto)

lemma ctt_prefix_subset'_trans: "x \<lesssim>`\<^sub>C y \<Longrightarrow> y \<lesssim>`\<^sub>C z \<Longrightarrow> x \<lesssim>`\<^sub>C z"
proof -
  have "\<exists> y. x \<lesssim>`\<^sub>C y \<and> y \<lesssim>`\<^sub>C z \<Longrightarrow> x \<lesssim>`\<^sub>C z"
  proof (induct rule:ctt_prefix_subset'.induct, auto)
    fix X Y :: "'e cttevent set"
    fix x y
    assume assm1: "x \<in> X"
    assume assm2: "CTT [] [X]`\<^sub>R \<lesssim>`\<^sub>C y"
    assume assm3: "y \<lesssim>`\<^sub>C CTT [] [Y]`\<^sub>R"
    obtain Z where "y = CTT [] [Z]`\<^sub>R"
      using assm2 ctt_prefix_subset'.elims(2) by blast
    then show "x \<in> Y"
      using assm1 assm2 assm3 ctt_prefix_subset'.simps(3) by auto
  next
    fix x xa xb y ya yb yc
    assume assm1: "\<exists>y. CTT xa xb \<lesssim>`\<^sub>C y \<and> y \<lesssim>`\<^sub>C CTT ya yb \<Longrightarrow> CTT xa xb \<lesssim>`\<^sub>C CTT ya yb"
    assume assm2: "CTT (x # xa) xb \<lesssim>`\<^sub>C yc"
    assume assm3: "yc \<lesssim>`\<^sub>C CTT (y # ya) yb"
    obtain yca ycb ycc where "yc = CTT (yca # ycb) ycc"
      by (metis assm2 ctt.exhaust ctt_prefix_subset'.simps(5) list.exhaust)
    then show "x \<subseteq>\<^sub>T y"
      using assm2 assm3 eventtockref_order_trans by auto
  next
    fix x xa xb y ya yb yc
    assume assm1: "\<exists>y. CTT xa xb \<lesssim>`\<^sub>C y \<and> y \<lesssim>`\<^sub>C CTT ya yb \<Longrightarrow> CTT xa xb \<lesssim>`\<^sub>C CTT ya yb"
    assume assm2: "CTT (x # xa) xb \<lesssim>`\<^sub>C yc"
    ...
    obtain yca ycb ycc where "yc = CTT (yca # ycb) ycc"
      by (metis assm2 ctt.exhaust ctt_prefix_subset'.simps(5) list.exhaust)
    then show "CTT xa xb \<lesssim>`\<^sub>C CTT ya yb"
      using assm1 assm2 assm3 by auto
  next
    fix vb vc va vd y
    assume assm1: "CTT (vb # vc) va \<lesssim>`\<^sub>C y" 
    assume assm2: "y \<lesssim>`\<^sub>C CTT [] vd"
    obtain ya where "y = CTT [] ya"
      using assm2 ctt_prefix_subset'.elims(2) by blast
    then show "False"
      using assm1 by auto
  ...
  next
    fix vb vd ve vc y
    assume assm1: "CTT [] [vb]`\<^sub>R \<lesssim>`\<^sub>C y" 
    assume assm2: " y \<lesssim>`\<^sub>C CTT (vd # ve) vc"
    obtain ya yb yc where "y = CTT (ya # yb) yc"
      by (metis assm1 assm2 ctt.exhaust ctt_prefix_subset'.simps(11) ctt_prefix_subset'.simps(7) ctt_prefix_subset'.simps(9) endevent.exhaust list.exhaust)
    then show "False"
      using assm1 by auto
  next
    fix vb va y
    assume assm1: "CTT [] [vb]`\<^sub>R \<lesssim>`\<^sub>C y" 
    assume assm2: "y \<lesssim>`\<^sub>C CTT va \<bullet>"
    obtain ya yb yc where "y = CTT (ya # yb) yc"
      by (metis assm1 assm2 ctt.exhaust ctt_prefix_subset'.simps(11) ctt_prefix_subset'.simps(7) ctt_prefix_subset'.simps(9) endevent.exhaust list.exhaust)
    then show "False"
      using assm1 by auto
  next
    fix v vb y
    assume assm1: "CTT v [vb]`\<^sub>R \<lesssim>`\<^sub>C y" 
    assume assm2: "y \<lesssim>`\<^sub>C CTT [] \<bullet>"
    obtain ya yb yc where "y = CTT (ya # yb) yc"
      by (metis assm1 assm2 ctt.exhaust ctt_prefix_subset'.simps(11)  ctt_prefix_subset'.simps(9) endevent.exhaust list.exhaust)
    then show "False"
      using assm2 by auto
  next
    fix vb va y
    assume assm1: "CTT [] [vb]`\<^sub>R \<lesssim>`\<^sub>C y" 
    assume assm2: "y \<lesssim>`\<^sub>C CTT va ETick"
    obtain ya yb yc where "y = CTT (ya # yb) yc"
      by (metis assm1 assm2 ctt.exhaust ctt_prefix_subset'.simps(11) ctt_prefix_subset'.simps(7) ctt_prefix_subset'.simps(9) endevent.exhaust list.exhaust)
    then show "False"
      using assm1 by auto
  next
    fix v vb y
    assume assm1: "CTT v [vb]`\<^sub>R \<lesssim>`\<^sub>C y" 
    assume assm2: "y \<lesssim>`\<^sub>C CTT [] ETick"
    obtain ya yb yc where "y = CTT (ya # yb) yc"
      by (metis assm1 assm2 ctt.exhaust ctt_prefix_subset'.simps(11)  ctt_prefix_subset'.simps(9) endevent.exhaust list.exhaust)
    then show "False"
      using assm2 by auto
  next
    fix vc vd vb y
    assume assm1: "CTT [] ETick \<lesssim>`\<^sub>C y" 
    assume assm2: "y \<lesssim>`\<^sub>C CTT (vc # vd) vb"
    have "y = CTT [] ETick"
      using assm1 ctt_prefix_subset'.elims(2) by auto
    then show "False"
      using assm2 by auto
  next
    fix va y
  ...
  next
    fix v y
    assume assm1: "CTT v ETick \<lesssim>`\<^sub>C y" 
    assume assm2: "y \<lesssim>`\<^sub>C CTT [] \<bullet>"
    obtain ya where "y = CTT ya ETick"
      using assm1 assm2 ctt_prefix_subset'.elims(2) by fastforce
    then show "False"
      using assm2 by auto
  next
    fix va vc y
    assume assm1: "CTT [] ETick \<lesssim>`\<^sub>C y" 
    assume assm2: "y \<lesssim>`\<^sub>C CTT va [vc]`\<^sub>R"
    obtain ya where "y = CTT [] ETick"
      using assm1 assm2 ctt_prefix_subset'.elims(2) by fastforce
    then show "False"
      using assm2 by auto
  next
    fix v vc y
    assume assm1: "CTT v ETick \<lesssim>`\<^sub>C y" 
    assume assm2: "y \<lesssim>`\<^sub>C CTT [] [vc]`\<^sub>R"
    obtain ya where "y = CTT [] ETick"
      using assm1 assm2 ctt_prefix_subset'.elims(2) by fastforce
    then show "False"
      using assm2 by auto
...
lemma ctt_prefix_subset'_antisym: "x \<lesssim>`\<^sub>C y \<Longrightarrow> y \<lesssim>`\<^sub>C x \<Longrightarrow> x = y"
  using ctt_prefix_subset'.elims(2) by (induct rule:ctt_prefix_subset'.induct,  auto simp add: eventtockref_order_antisym)

lemma ctt_prefix'_imp_prefix_subset': "x \<le>`\<^sub>C y \<Longrightarrow> x \<lesssim>`\<^sub>C y"
  apply (induct rule:ctt_prefix_subset'.induct, auto)
  using eventtockref_order_refl apply blast
  using ctt_prefix'.elims(2) apply auto
  done

subsection {* Initial sequences of tocks *}

fun ntock' ::"'e cttevent set \<Rightarrow> nat \<Rightarrow> 'e eventtockref list set" where
  "ntock' X 0 = {[]}" |
  "ntock' X (Suc n) = {t. \<exists> Y s. Tock \<notin> Y \<and> Y \<subseteq> X \<and> t = [Y]`\<^sub>T # s \<and> s \<in> ntock' X n} \<union> {t. \<exists> s. t = TTock # s \<and> s \<in> ntock' X n} "

lemma ntock'_subset: "X \<subseteq> Y \<Longrightarrow> ntock' X n \<subseteq> ntock' Y n"
  by (induct rule:ntock.induct, clarsimp+, blast)

lemma ntock'_remove_Tock: "ntock X n = ntock {x. x \<in> X \<and> x \<noteq> Tock} n"
  by (induct n, clarsimp+, auto)
  
definition add_pretocks' :: "'e cttevent set \<Rightarrow> 'e eventtockref list set \<Rightarrow> 'e eventtockref list set" where
  "add_pretocks' X Y = {t. \<exists> n s x. t = s @ x \<and> x \<in> Y \<and> s \<in> ntock' X n}"

lemma in_add_pretocks'_decompose: "x \<in> add_pretocks' X Y \<Longrightarrow> \<exists> s y. s \<in> add_pretocks' X {[]} \<and> y \<in> Y \<and> x = s @ y "
  unfolding add_pretocks'_def by auto

lemma concat_in_add_pretocks': "s \<in> add_pretocks' X {[]} \<Longrightarrow> y \<in> Y \<Longrightarrow> s @ y \<in> add_pretocks' X Y"
  unfolding add_pretocks'_def by auto

lemma add_pretocks'_subset1: "X1 \<subseteq> X2 \<Longrightarrow> add_pretocks' X1 Y \<subseteq> add_pretocks' X2 Y"
  unfolding add_pretocks'_def using ntock'_subset by blast

lemma add_pretocks'_subset2: "Y1 \<subseteq> Y2 \<Longrightarrow> add_pretocks' X Y1 \<subseteq> add_pretocks' X Y2"
  unfolding add_pretocks'_def by blast

lemma self_in_add_pretocks': "Y \<subseteq> add_pretocks' X Y"
  unfolding add_pretocks'_def by (auto, rule_tac x="0" in exI, auto)

subsection {* Operators *}

subsubsection {* Div *}

definition DivCTT' :: "'e ctt set" ("div`\<^sub>C") where
  "div`\<^sub>C = {CTT [] \<bullet>}"

subsubsection {* Stop *}

definition StopCTT' :: "'e ctt set" ("STOP`\<^sub>C") where
  "STOP`\<^sub>C = {(CTT t X) |t X. t \<in> add_pretocks' {x. x \<noteq> Tock} {[]} \<and> ((\<exists> X'. Tock \<notin> X' \<and> X = [X']`\<^sub>R) \<or> X = \<bullet>)}"

subsubsection {* Skip *}

definition SkipCTT' :: "'e ctt set" ("SKIP`\<^sub>C") where
  "SKIP`\<^sub>C = {(CTT [] X) | X. (\<exists> X'. Tick \<notin> X' \<and> X = [X']`\<^sub>R) \<or> X = \<bullet> \<or> X = ETick}"

subsubsection {* Prefix *}

definition PrefixCTT' :: "'e \<Rightarrow> 'e ctt set \<Rightarrow> 'e ctt set" where
  "PrefixCTT' e P =
    {(CTT t X) | t X. t \<in> add_pretocks' {x. x \<noteq> Event e \<and> x \<noteq> Tock} {[]} \<and> (\<exists> X'. Tock \<notin> X' \<and> Event e \<notin> X' \<and> X = [X']`\<^sub>R) \<or> X = \<bullet>}
     \<union> {(CTT (t @ s) X) | t s X. (CTT s X) \<in> P \<and> t \<in> add_pretocks' {x. x \<noteq> Event e \<and> x \<noteq> Tock} {[[e]`\<^sub>E]} }"

subsection {* Internal Choice *}

definition IntChoiceCTT' :: "'e ctt set \<Rightarrow> 'e ctt set \<Rightarrow> 'e ctt set" (infixl "\<sqinter>`\<^sub>C" 70) where
  "P \<sqinter>`\<^sub>C Q = P \<union> Q"
...
definition WaitCTT' :: "nat \<Rightarrow> 'e ctt set" ("wait`\<^sub>C[_]") where
  "wait`\<^sub>C[n] =
    {(CTT t \<bullet>) | t. t \<in> add_pretocks' {x. x \<noteq> Tock} {[]} \<and> length (filter (\<lambda> x. x = TTock) t) \<le> n}
     \<union> {(CTT t [X]`\<^sub>R) | t X. t \<in> add_pretocks' {x. x \<noteq> Tock} {[]} \<and> length (filter (\<lambda> x. x = TTock) t) < n}
     \<union> {(CTT t ETick) | t. t \<in> add_pretocks' {x. x \<noteq> Tock} {[]} \<and> length (filter (\<lambda> x. x = TTock) t) = n}"

subsection {* External Choice *}

definition longest_eventtockref_prefix :: "'e eventtockref list \<Rightarrow> 'e eventtockref list" where
  "longest_eventtockref_prefix x = (THE x'. (CTT x' \<bullet>) \<le>`\<^sub>C (CTT x \<bullet>) \<and> x' \<in> add_pretocks' UNIV ({[]})
      \<and> (\<forall> x''. (CTT x'' \<bullet>) \<le>`\<^sub>C (CTT x \<bullet>) \<and> x'' \<in> add_pretocks' UNIV ({[]}) \<longrightarrow> (CTT x'' \<bullet>) \<le>`\<^sub>C (CTT x' \<bullet>)))"
...
definition ExtChoiceCTT_aux1' :: "'e ctt set \<Rightarrow> 'e ctt set \<Rightarrow> 'e ctt set" where
  "ExtChoiceCTT_aux1' P Q = {CTT \<rho> [X]`\<^sub>R | \<rho> X. (CTT \<rho> [X]`\<^sub>R) \<in> P \<inter> Q}"

definition ExtChoiceCTT_aux2' :: "'e ctt set \<Rightarrow> 'e ctt set \<Rightarrow> 'e ctt set" where
  "ExtChoiceCTT_aux2' P Q = {\<rho>. \<exists> \<sigma> X Y. (\<rho> = CTT \<sigma> X \<or> \<rho> = CTT \<sigma> Y) \<and>
    \<sigma> \<in> add_pretocks' UNIV {[]} \<and>
    CTT \<sigma> X \<in> Q \<and> (Y = \<bullet> \<or> Y = ETick) \<and>
    CTT \<sigma> Y \<in> P \<and> (X = \<bullet> \<or> X = ETick)}"

definition ExtChoiceCTT_aux3' :: "'e ctt set \<Rightarrow> 'e ctt set \<Rightarrow> 'e ctt set" where
  "ExtChoiceCTT_aux3' P Q = {\<rho>. \<exists> \<sigma> \<tau> X Y. (\<rho> = CTT \<sigma> X \<or> \<rho> = CTT \<tau> Y) \<and>
    CTT \<sigma> X \<in> Q \<and> \<sigma> \<noteq> longest_eventtockref_prefix \<sigma> \<and>
    CTT \<tau> Y \<in> P \<and> \<tau> \<noteq> longest_eventtockref_prefix \<tau> \<and>
    longest_eventtockref_prefix \<sigma> = longest_eventtockref_prefix \<tau>}"

definition ExtChoiceCTT_aux' :: "'e ctt set \<Rightarrow> 'e ctt set \<Rightarrow> 'e ctt set" where
  "ExtChoiceCTT_aux' P Q = ExtChoiceCTT_aux1' P Q \<union> ExtChoiceCTT_aux2' P Q \<union> ExtChoiceCTT_aux2' Q P"

lemma ExtChoiceCTT_aux'_comm: "ExtChoiceCTT_aux' P Q = ExtChoiceCTT_aux' Q P"
  unfolding ExtChoiceCTT_aux'_def ExtChoiceCTT_aux1'_def by auto

lemma ExtChoiceCTT_aux1'_union_dist1: "ExtChoiceCTT_aux1' (P \<union> Q) R = (ExtChoiceCTT_aux1' P R) \<union> (ExtChoiceCTT_aux1' Q R)"
  unfolding ExtChoiceCTT_aux1'_def by auto

lemma ExtChoiceCTT_aux1'_union_dist2: "ExtChoiceCTT_aux1' R (P \<union> Q) = (ExtChoiceCTT_aux1' R P) \<union> (ExtChoiceCTT_aux1' R Q)"
  unfolding ExtChoiceCTT_aux1'_def by auto

lemma ExtChoiceCTT_aux2'_union_dist1: "ExtChoiceCTT_aux2' (P \<union> Q) R = (ExtChoiceCTT_aux2' P R) \<union> (ExtChoiceCTT_aux2' Q R)"
  unfolding ExtChoiceCTT_aux2'_def by (safe, rule_tac x="\<sigma>" in exI, rule_tac x="\<bullet>" in exI, auto)

lemma ExtChoiceCTT_aux2'_union_dist2: "ExtChoiceCTT_aux2' R (P \<union> Q) = (ExtChoiceCTT_aux2' R P) \<union> (ExtChoiceCTT_aux2' R Q)"
  unfolding ExtChoiceCTT_aux2'_def by (safe, rule_tac x="\<sigma>" in exI, rule_tac x="\<bullet>" in exI, auto)

lemma ExtChoiceCTT_aux1'_assoc: "ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux1' Q R) = ExtChoiceCTT_aux1' (ExtChoiceCTT_aux1' P Q) R"
  unfolding ExtChoiceCTT_aux1'_def by (auto)

lemma ExtChoiceCTT_aux2'_assoc: "ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux2' Q R) = ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' P Q) R"
  unfolding ExtChoiceCTT_aux2'_def proof
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="ETick" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="ETick" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="ETick" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="ETick" in exI, simp)
  apply auto
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<sigma>'" in exI, rule_tac x="ETick" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<sigma>'" in exI, rule_tac x="ETick" in exI, simp)
  done

lemma ExtChoiceCTT_aux3'_assoc: "ExtChoiceCTT_aux3' P (ExtChoiceCTT_aux3' Q R) = ExtChoiceCTT_aux3' (ExtChoiceCTT_aux3' P Q) R"
  unfolding ExtChoiceCTT_aux3'_def ExtChoiceCTT_aux2'_def
  apply (safe)
  apply fastforce
  apply fastforce
  apply fastforce
  ...
  apply fastforce
  apply fastforce
  fix x \<sigma> X \<tau> Y \<sigma>' Xa \<tau>' Ya
  assume assm1: "CTT \<tau> \<bullet> \<in> P"
  assume assm2: "CTT \<sigma>' \<bullet> \<in> R"
  assume assm3: "CTT \<tau>' \<bullet> \<in> Q"
  assume assm4: "longest_eventtockref_prefix \<sigma>' = longest_eventtockref_prefix \<tau>"
  assume assm5: "longest_eventtockref_prefix \<sigma>' = longest_eventtockref_prefix \<tau>'"
  show "\<exists>\<sigma> X. CTT \<sigma>' \<bullet> = CTT \<sigma> X \<and>
             CTT \<sigma> X \<in> R \<and>
             (\<exists>\<tau> Y. CTT \<tau> Y
                    \<in> {CTT \<sigma> X |\<sigma> X.
                        CTT \<sigma> X \<in> Q \<and>
                        (\<exists>\<tau> Y. CTT \<tau> Y \<in> P \<and>
                               longest_eventtockref_prefix \<sigma> = longest_eventtockref_prefix \<tau> \<and> (Y = \<bullet> \<or> Y = ETick)) \<and>
                        (X = \<bullet> \<or> X = ETick)} \<union>
                       {CTT \<sigma> X |\<sigma> X.
                        CTT \<sigma> X \<in> P \<and>
                        (\<exists>\<tau> Y. CTT \<tau> Y \<in> Q \<and>
                               longest_eventtockref_prefix \<sigma> = longest_eventtockref_prefix \<tau> \<and> (Y = \<bullet> \<or> Y = ETick)) \<and>
                        (X = \<bullet> \<or> X = ETick)} \<and>
                    longest_eventtockref_prefix \<sigma> = longest_eventtockref_prefix \<tau> \<and> (Y = \<bullet> \<or> Y = ETick)) \<and>
             (X = \<bullet> \<or> X = ETick)"
    apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp add: assm2)
    apply (rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, safe, auto simp add: assm3)
    using assm4 assm5 apply (rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, auto simp add: assm1)+
    done
next
  fix x \<sigma> X \<tau> Y \<sigma>' Xa \<tau>' Ya
  assume assm1: "CTT \<tau> \<bullet> \<in> P"
  assume assm2: "CTT \<sigma>' \<bullet> \<in> R"
  assume assm3: "CTT \<tau>' ETick \<in> Q"
  assume assm4: "longest_eventtockref_prefix \<sigma>' = longest_eventtockref_prefix \<tau>"
  assume assm5: "longest_eventtockref_prefix \<sigma>' = longest_eventtockref_prefix \<tau>'"
  show "\<exists>\<sigma> X. CTT \<sigma>' \<bullet> = CTT \<sigma> X \<and>
             CTT \<sigma> X \<in> R \<and>
             ...
             (X = \<bullet> \<or> X = ETick)"
    apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp add: assm2)
    apply (rule_tac x="\<tau>'" in exI, rule_tac x="ETick" in exI, safe, auto simp add: assm3)
    using assm4 assm5 apply (rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, auto simp add: assm1)+
    done
next
  fix x \<sigma> X \<tau> Y \<sigma>' Xa \<tau>' Ya
  assume assm1: "CTT \<tau> ETick \<in> P"
  assume assm2: "CTT \<sigma>' \<bullet> \<in> R"
  assume assm3: "CTT \<tau>' \<bullet> \<in> Q"
  assume assm4: "longest_eventtockref_prefix \<sigma>' = longest_eventtockref_prefix \<tau>"
  assume assm5: "longest_eventtockref_prefix \<sigma>' = longest_eventtockref_prefix \<tau>'"
  show "\<exists>\<sigma> X. CTT \<sigma>' \<bullet> = CTT \<sigma> X \<and>
    ...
    apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp add: assm2)
    apply (rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, safe, auto simp add: assm3)
    using assm4 assm5 apply (rule_tac x="\<tau>" in exI, rule_tac x="ETick" in exI, auto simp add: assm1)+
    done
next
  fix x \<sigma> X \<tau> Y \<sigma>' Xa \<tau>' Ya
  assume assm1: "CTT \<tau> ETick \<in> P"
...
next
                      ...
                      apply (rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, simp+)+
  apply safe
                      apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp) apply (rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, safe) apply (rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, simp+)+

                     



lemma ExtChoiceCTT_aux2'_permute:
  "(\<lambda> P Q. ExtChoiceCTT_aux2' P Q \<union> ExtChoiceCTT_aux2' Q P) = ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' Q R) P"
  unfolding ExtChoiceCTT_aux2'_def apply (safe)
  apply (rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, simp) apply (rule_tac x="\<sigma>" in exI, rule_tac x="\<bullet>" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="ETick" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="ETick" in exI, simp)
  apply (rule_tac x="\<sigma>'" in exI, rule_tac x="ETick" in exI, simp, rule_tac x="\<tau>'" in exI, rule_tac x="\<bullet>" in exI, simp, rule_tac x="\<tau>" in exI, rule_tac x="\<bullet>" in exI, simp)
...
lemma ExtChoiceCTT_aux'_assoc:
  "ExtChoiceCTT_aux' (ExtChoiceCTT_aux' P Q) R = ExtChoiceCTT_aux' P (ExtChoiceCTT_aux' Q R)"
  (is "?lhs = ?rhs")
proof -
  have "?lhs = ExtChoiceCTT_aux1' (ExtChoiceCTT_aux1' P Q \<union> ExtChoiceCTT_aux2' P Q \<union> ExtChoiceCTT_aux2' Q P) R \<union>
    ExtChoiceCTT_aux2' (ExtChoiceCTT_aux1' P Q \<union> ExtChoiceCTT_aux2' P Q \<union> ExtChoiceCTT_aux2' Q P) R \<union>
    ExtChoiceCTT_aux2' R (ExtChoiceCTT_aux1' P Q \<union> ExtChoiceCTT_aux2' P Q \<union> ExtChoiceCTT_aux2' Q P)"
    unfolding ExtChoiceCTT_aux'_def by simp
  also have "... = ExtChoiceCTT_aux1' (ExtChoiceCTT_aux1' P Q) R
                    \<union> ExtChoiceCTT_aux1' (ExtChoiceCTT_aux2' P Q) R
                    \<union> ExtChoiceCTT_aux1' (ExtChoiceCTT_aux2' Q P) R
                    \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux1' P Q) R
                    \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' P Q) R
                    \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' Q P) R
                    \<union> ExtChoiceCTT_aux2' R (ExtChoiceCTT_aux1' P Q)
                    \<union> ExtChoiceCTT_aux2' R (ExtChoiceCTT_aux2' P Q)
                    \<union> ExtChoiceCTT_aux2' R (ExtChoiceCTT_aux2' Q P)"
    by (simp add: ExtChoiceCTT_aux1'_union_dist1 ExtChoiceCTT_aux2'_union_dist1 ExtChoiceCTT_aux2'_union_dist2 semigroup.assoc sup.semigroup_axioms)
  also have "... = ExtChoiceCTT_aux1' (ExtChoiceCTT_aux1' P Q) R
                    \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' P Q) R
                    \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' Q P) R
                    \<union> ExtChoiceCTT_aux2' R (ExtChoiceCTT_aux2' P Q)
                    \<union> ExtChoiceCTT_aux2' R (ExtChoiceCTT_aux2' Q P)"
    unfolding ExtChoiceCTT_aux1'_def ExtChoiceCTT_aux2'_def by safe

  also have "... = ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux1' Q R)
                    \<union> ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux2' Q R)
                    \<union> ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux2' R Q)
                    \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' Q R) P
                    \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' R Q) P"
  proof -
    have "ExtChoiceCTT_aux1' (ExtChoiceCTT_aux1' P Q) R = ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux1' Q R)"
      by (simp add: ExtChoiceCTT_aux1'_assoc)
    have "ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' P Q) R = ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux2' Q R)"
      by (simp add: ExtChoiceCTT_aux2'_assoc)
    have "ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' Q P) R = ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux2' R Q)"
      unfolding ExtChoiceCTT_aux2'_def apply auto
    have "ExtChoiceCTT_aux2' R (ExtChoiceCTT_aux2' P Q) = ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' Q R) P" sorry
    have "ExtChoiceCTT_aux2' R (ExtChoiceCTT_aux2' Q P) = ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' R Q) P"
      by (simp add: ExtChoiceCTT_aux2'_assoc)

  have "ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux1' Q R)
     \<union> ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux2' Q R)
     \<union> ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux2' R Q)
     \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' Q R) P
     \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' R Q) P
      = ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux1' Q R)
     \<union> ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux2' Q R)
     \<union> ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux2' R Q)
     \<union> ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux1' Q R)
     \<union> ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux2' Q R)
     \<union> ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux2' R Q)
     \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux1' Q R) P
     \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' Q R) P
     \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' R Q) P"
    unfolding ExtChoiceCTT_aux1'_def ExtChoiceCTT_aux2'_def by safe
  ...
  have "ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux1' Q R)
     ...
     \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' Q R) P
     \<union> ExtChoiceCTT_aux2' (ExtChoiceCTT_aux2' R Q) P
    = ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux1' Q R \<union> ExtChoiceCTT_aux2' Q R \<union> ExtChoiceCTT_aux2' R Q) \<union>
    ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux1' Q R \<union> ExtChoiceCTT_aux2' Q R \<union> ExtChoiceCTT_aux2' R Q) \<union>
    ExtChoiceCTT_aux2' (ExtChoiceCTT_aux1' Q R \<union> ExtChoiceCTT_aux2' Q R \<union> ExtChoiceCTT_aux2' R Q) P"
    by (simp add: ExtChoiceCTT_aux1'_union_dist2 ExtChoiceCTT_aux2'_union_dist1 ExtChoiceCTT_aux2'_union_dist2 sup_assoc)
  

  have "ExtChoiceCTT_aux1' P (ExtChoiceCTT_aux1' Q R \<union> ExtChoiceCTT_aux2' Q R \<union> ExtChoiceCTT_aux2' R Q) \<union>
    ExtChoiceCTT_aux2' P (ExtChoiceCTT_aux1' Q R \<union> ExtChoiceCTT_aux2' Q R \<union> ExtChoiceCTT_aux2' R Q) \<union>
    ExtChoiceCTT_aux2' (ExtChoiceCTT_aux1' Q R \<union> ExtChoiceCTT_aux2' Q R \<union> ExtChoiceCTT_aux2' R Q) P = ?rhs"
    unfolding ExtChoiceCTT_aux'_def by simp
