...
lemma TT3w_Hiding:
  "TT1 P \<Longrightarrow> TT3w P \<Longrightarrow> TT3w (P \<setminus>\<^sub>C X)"
  unfolding TT3w_def HidingTT_def
proof (safe, simp_all)
  fix p
  ...
  next
    ...
    have 2: "s \<in> {s. [Y\<union>{Tick}]\<^sub>R # [Tock]\<^sub>E # s \<in> P}"
      using TT3w_TT1_add_Tick_ref_Tock TT3w_def case_assms(1) case_assms(2) case_assms(3) by blast
...
lemma TT3_Hiding: "TT1 P \<Longrightarrow> TT3 P \<Longrightarrow> TT3 (P \<setminus>\<^sub>C X)"
  by (simp add: TT1_Hiding TT1_TT3w_equiv_TT3 TT3w_Hiding)
