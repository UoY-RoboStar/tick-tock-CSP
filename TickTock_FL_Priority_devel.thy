...
lemma fl_le_TT1c_ES_Event:
  assumes "fl \<le> \<langle>([{x. x \<notin> ES}]\<^sub>\<F>\<^sub>\<L>,Event ev)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>" "[[Event ev]\<^sub>E] \<in> P" "[[ES]\<^sub>R] \<in> P" "Event ev \<notin> ES" "TT1c P"
...
lemma prirelRef_concat_dist1:
  ...
  using not_prirelRef_concat_refTock apply fastforce
      ...
      apply (metis append_Cons list.distinct(1) not_prirelRef_concat_refTock')
     apply (induct p xs ys s P rule:prirelRef.induct, auto)
          apply (metis ttWF.simps(1) ttWF.simps(4) ttWF.simps(6) ttWF.simps(8) cttevent.exhaust neq_Nil_conv)
         apply (metis append.left_neutral ttWF.simps(4) ttWF.simps(6) ttWF.simps(8) ttWF_prefix_is_ttWF cttevent.exhaust neq_Nil_conv)
  ...
  using not_prirelRef_concat_refTock apply fastforce
    ...
    apply (metis Cons_eq_appendI neq_Nil_conv not_prirelRef_concat_refTock')
   apply (induct p xs ys s P rule:prirelRef.induct, auto)
        apply (metis ttWF.simps(1) ttWF.simps(10) ttWF.simps(4) ttWF.simps(6) cttevent.exhaust neq_Nil_conv)
       apply (metis ttWF.simps(1) ttWF.simps(10) ttWF.simps(4) ttWF.simps(6) cttevent.exhaust neq_Nil_conv)
      apply (metis Cons_eq_appendI neq_Nil_conv not_prirelRef_concat_refTock)
     apply (metis Cons_eq_appendI neq_Nil_conv not_prirelRef_concat_refTock)
    apply (metis Cons_eq_appendI neq_Nil_conv not_prirelRef_concat_refTock')
...
apply (induct p xs ys s P rule:prirelRef.induct, auto) 
  ...
  by (metis ttWF.simps(1) ttWF.simps(10) ttWF.simps(4) ttWF.simps(6) cttevent.exhaust neq_Nil_conv)
...
lemma TTwf_1c_3_imp_flt2cttobs_FL1_mod:
  assumes "x \<in> P" 
...
next
  ...
  then have "\<exists>ar. prirelRef p ar xs [] P \<and> xs \<in> P"
    ...
    using TT1c_def ctt_prefix_concat
    using TT1c_healthy by blast
        ...
        next
          ...
          then show ?thesis using ObsEvent Event False ES
            ...
            apply (intro exI[where x="{z. z \<le> \<langle>([{x. x \<notin> ES}]\<^sub>\<F>\<^sub>\<L>,Event ev)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>}"], auto)
              ...
              using FL1_def dual_order.trans apply blast
          using ObsEvent Nil by (simp add: fl_le_TT1c_ES_Event)
      qed
      next
        case Tock
  ...
  next
    ...
    from yys have ys_y_inP:"ys @ [y] \<in> P" using TT1c_def
        ...
        proof (cases e2)
          ...
          proof (cases "maximal(p,Event e3)")
           ...
           then have "
             ...
             using flt2cttobs_strongFL_subset TT3_trace_cons_imp_cons
             by (smt Un_iff mem_Collect_eq subset_iff)
           then have "
                    ys @ [[e1]\<^sub>E] @ [[Event e3]\<^sub>E] = flt2cttobs(fl @\<^sub>\<F>\<^sub>\<L> \<langle>(last fl,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)
                    \<and> flt2goodAcceptance fl p \<and> flt2goodTock fl 
          ...
          next
            ...
            have "flt2cttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P"
              using fl Z
              by (metis (mono_tags, lifting) Nil_is_append_conv append.assoc flt2cttobs_fl_acceptance last_snoc not_Cons_self2)
            then have "flt2cttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P"
            proof -
              have "flt2cttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) 
                    =
                    flt2cttobs (fl) @ flt2cttobs(\<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
                using fl e2 Event last_fl 
                by (metis (no_types, lifting) bullet_right_zero2 butlast_last_cons2_FL fl_cons_acceptance_consFL flt2cttobs_acceptance_cons_eq_list_cons)
              also have "... = ys @ [[e1]\<^sub>E] @ flt2cttobs(\<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
                using fl by auto
            ...
            then have flt2cttobs_strongFL_subset:"{flt2cttobs fl\<^sub>0|fl\<^sub>0. fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<or> fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>)} \<subseteq> P"
              using False strong_less_eq_fltrace_imp_flt2cttobs_ctt TT1c_def TT1c_healthy Event cttevent.simps(3) e1 e2 fl flt2cttobs_last_non_tock last_fl last_snoc yys.prems(2)
              using \<open>flt2cttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P\<close> by blast

            from fl have fl_e3: "ys @ [[e1]\<^sub>E] @ [[Event e3]\<^sub>E] = flt2cttobs fl @ [[Event e3]\<^sub>E]
                    \<and> flt2goodAcceptance fl p \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2cttobs fl\<^sub>0 |fl\<^sub>0. fl\<^sub>0 \<in> x} \<subseteq> P \<and> fl \<in> x)"
              using ys_e1_x by auto
            also have "... =
                    (ys @ [[e1]\<^sub>E] @ [[Event e3]\<^sub>E] = flt2cttobs(fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)
                    \<and> flt2goodAcceptance fl p \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2cttobs fl\<^sub>0 |fl\<^sub>0. fl\<^sub>0 \<in> x} \<subseteq> P \<and> fl \<in> x))"
              proof -
                from fl have "ys @ [[e1]\<^sub>E] = flt2cttobs fl"
                  by auto
                then have "List.last(flt2cttobs fl) = [e1]\<^sub>E"
                  by (metis last_snoc)
                then have "flt2cttobs fl @ [[Event e3]\<^sub>E] = flt2cttobs(fl) @ flt2cttobs(\<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
                  using Z by auto
                also have "... = ys @ [[e1]\<^sub>E] @ flt2cttobs(\<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
                  by (simp add: fl)
                also have "... = flt2cttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
                  using fl last_fl flt2cttobs_last_non_tock
                  by (metis (no_types, lifting) \<open>flt2cttobs fl @ flt2cttobs \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> = ys @ [[e1]\<^sub>E] @ flt2cttobs \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> bullet_right_zero2 butlast_last_cons2_FL fl_cons_acceptance_consFL flt2cttobs_acceptance_cons_eq_list_cons)
              ...
              proof -
                assume "ys @ [[e1]\<^sub>E] @ [[Event e3]\<^sub>E] = flt2cttobs fl @ [[Event e3]\<^sub>E] \<and> ys @ [[e1]\<^sub>E] @ [[Event e3]\<^sub>E] = flt2cttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> flt2goodAcceptance fl p \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2cttobs fl\<^sub>0 |fl\<^sub>0. fl\<^sub>0 \<in> x} \<subseteq> P \<and> fl \<in> x)"
                then obtain FF :: "'a cttevent fltrace set" where
                  f1: "ys @ [[e1]\<^sub>E] @ [[Event e3]\<^sub>E] = flt2cttobs fl @ [[Event e3]\<^sub>E] \<and> ys @ [[e1]\<^sub>E] @ [[Event e3]\<^sub>E] = flt2cttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{c. c \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> flt2goodAcceptance fl p \<and> flt2goodTock fl \<and> FLTick0 Tick FF \<and> FL1 FF \<and> {flt2cttobs f |f. f \<in> FF} \<subseteq> P \<and> fl \<in> FF"
                  by blast
                ...
                have "Event e3 \<in>\<^sub>\<F>\<^sub>\<L> [{c. c \<notin> Z}]\<^sub>\<F>\<^sub>\<L>"
                  by (simp add: Z)
                then have "\<exists>F. ((FL1 (F \<union> {f. f \<le>\<^sub>\<F>\<^sub>\<L> fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{c. c \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L> \<or> f \<le>\<^sub>\<F>\<^sub>\<L> fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{c. c \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,Finite_Linear_Model.last fl\<rangle>\<^sub>\<F>\<^sub>\<L>}) \<and> {flt2cttobs f |f. f \<in> F} \<subseteq> P) \<and> fl \<in> F) \<and> FLTick0 Tick F"
            ...
            then have "
              ...
              proof -
                ...
                assume a1: "FL1 (xb \<union> {fl\<^sub>0. fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L> \<or> fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>})"
                  assume a2: "{flt2cttobs fl\<^sub>0 |fl\<^sub>0. fl\<^sub>0 \<in> xb} \<subseteq> P"
                  assume a3: "fl \<in> xb"
                  ...
                  then show "\<exists>F. FLTick0 Tick (F \<union> {f. f \<le>\<^sub>\<F>\<^sub>\<L> fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{c. c \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L> \<or> f \<le>\<^sub>\<F>\<^sub>\<L> fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{c. c \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>}) \<and> FL1 (F \<union> {f. f \<le>\<^sub>\<F>\<^sub>\<L> fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{c. c \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L> \<or> f \<le>\<^sub>\<F>\<^sub>\<L> fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{c. c \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>}) \<and> {flt2cttobs f |f. f \<in> F} \<subseteq> P \<and> fl \<in> F"
           ...
           then have "
                    ...
                    \<and> (\<exists>x. FLTick0 Tick (x \<union> {fl\<^sub>0. fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<or> fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)}) 
                        \<and> FL1(x \<union> {fl\<^sub>0. fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<or> fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)}) 
                        \<and> {flt2cttobs fl\<^sub>0 |fl\<^sub>0. fl\<^sub>0 \<in> x \<union> {fl\<^sub>0. fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<or> fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)}} \<subseteq> P \<and> fl \<in> x)"    
             using flt2cttobs_strongFL_subset
             by (smt Un_iff mem_Collect_eq subset_iff)
           then have "
                    ys @ [[e1]\<^sub>E] @ [[Event e3]\<^sub>E] = flt2cttobs(fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)
                    ...
                    \<and> (\<exists>x. FLTick0 Tick (x \<union> {fl\<^sub>0. fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<or> fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)}) 
                        \<and> FL1(x \<union> {fl\<^sub>0. fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<or> fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)}) 
                        \<and> {flt2cttobs fl\<^sub>0 |fl\<^sub>0. fl\<^sub>0 \<in> x \<union> {fl\<^sub>0. fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<or> fl\<^sub>0 \<le>\<^sub>\<F>\<^sub>\<L> (fl @\<^sub>\<F>\<^sub>\<L> \<langle>([{x. x \<notin> Z}]\<^sub>\<F>\<^sub>\<L>,Event e3)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)}} \<subseteq> P 
                        \<and> fl \<in> x
...
(*
lemma flt2cttobs_exists_flt2goodS_for_ttWF_TT3_trace:
...
next
  fix e \<sigma>
  assume hyp:"(TT3_trace \<sigma> \<Longrightarrow> \<exists>zr. flt2cttobs zr = \<sigma> \<and> flt2goodS p zr)"
...
next
  ...
  assume assm1:"ttWF (flt2cttobs zr)"
  assume assm2:"Tock \<notin> X"
  assume assm3:"TT3_trace (flt2cttobs zr)"
...
lemma xp:
  ...
  using assms  apply (induct p x zr s P rule:prirelRef.induct, auto)
   ...
   apply (metis TT3_trace.simps(2) ttWF.simps(2) flt2cttobs_exists_flt2goodS_for_ttWF_TT3_trace insert_Nil prirelRef.simps(2))
  oops*)
...
lemma FL2_ctt2fl:
  ...
  (* Requires property on P, namely TT2, to show this now. *)
  oops
...
lemma
  assumes "ttWF (flt2cttobs xs)"
  shows "ttWF ((flt2cttobs xs) @ (flt2cttobs \<langle>x,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>))"
  nitpick
...
lemma
  assumes "prirelRef p xs ys [] P" "ttWF xs" "ttWF ys" "ttWF x" "ttWF y" "TTwf P" "ys \<in> P" "TT1c P"
...
lemma prirelRef_extend_rhs_ttWF:
  ...
  by (metis ttWF.simps(10) ttWF.simps(4) ttWF.simps(6) cttevent.exhaust list.exhaust snoc_eq_iff_butlast)+
*)
...
lemma prirelRef_extend_both_events_non_maximal_ttWF:
  ...
  by (metis append_Nil ttWF.simps(10) ttWF.simps(4) ttWF.simps(6) ttWF_prefix_is_ttWF cttevent.exhaust list.exhaust)+

(* FIXME 
lemma flt2cttobs_exists_for_ttWF_TT3_trace:
  assumes "ttWF fl" "TT3_trace fl"
...
next
  fix e \<sigma>
  assume hyp:"(TT3_trace \<sigma> \<Longrightarrow> \<exists>zr. flt2cttobs zr = \<sigma>)"
  assume assm1:"ttWF \<sigma>"
  assume assm2:"TT3_trace ([Event e]\<^sub>E # \<sigma>)"
...
lemma flt2cttobs_is_TT3_trace [simp]:
  "TT3_trace (flt2cttobs xs)"
  apply (induct xs)
  ...
  proof (cases "flt2goodTock xs")
    ...
    then show ?thesis using 3
        ...
        (*  proof (cases y)
          ...
          next
           ...
           (* then obtain yR where yR:"flt2cttobs(\<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>) = [[yR]\<^sub>R]"
              by simp
            then have "ttWF (flt2cttobs (ys &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>))"
              by (meson "3.prems"(4) FLTick0_def assms(3) flt2cttobs_is_ttWF)
      ...
      next
        ...
        have "ttWF (flt2cttobs (ys &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>))"
          by (meson "3.prems"(4) FLTick0_def assms(3) flt2cttobs_is_ttWF)
        then have "ttWF ((flt2cttobs ys) @ [[yR]\<^sub>R])"
  ...
  next
   ...
   (* proof (cases y)
    ...
    next
      ...
      then obtain yR where yR:"flt2cttobs(\<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>) = [[yR]\<^sub>R]"
        by simp
      then have ttWF_ys_y:"ttWF (flt2cttobs (ys &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>))"
        by (meson "3.prems"(4) FLTick0_def assms(3) flt2cttobs_is_ttWF)
      then show ?thesis
      proof (cases "flt2goodTock ys")
        ...
        then have "prirelRef p (flt2cttobs xs) ((flt2cttobs ys) @ [[yR]\<^sub>R]) [] P"
          ...
          by (metis ttWF_ys_y flt2cttobs_cons_acceptance_eq_cons yR)
        then show ?thesis
...
next
  ...
  then show ?case
    proof (cases "yEvent = Tock")
      ...
      then have "ttWF (flt2cttobs (ys) @ [[yR]\<^sub>R,[Tock]\<^sub>E])"
        by (metis "4.prems"(4) FLTick0_def True assms(3) flt2_ys_y flt2cttobs_is_ttWF)
      then have "prirelRef p (flt2cttobs (xs) @ [[xR]\<^sub>R,[Tock]\<^sub>E]) (flt2cttobs (ys) @ [[yR]\<^sub>R,[Tock]\<^sub>E]) [] P"
        ...
        using yR
        by (metis TT3_trace.simps(3) flt2cttobs_is_TT3_trace xR)
    ...
    next
      ...
      then have ttWF_ys_y:"ttWF (flt2cttobs (ys &\<^sub>\<F>\<^sub>\<L> \<langle>y,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>))"
        using 4 
        by (meson FLTick0_def flt2cttobs_is_ttWF)

      then have ttWF_xs_x:"ttWF (flt2cttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>x,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>))"
        using 4 
        using prirel_rhs_tickWF_imp_lhs_tickWF
        by (metis FLTick0_def flt2cttobs_is_ttWF)
      ...
      proof (cases "maximal(p,yEvent)")
        case True
        then show ?thesis
          by (metis \<open>flt2cttobs \<langle>x,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> = [[yEvent]\<^sub>E]\<close> \<open>flt2cttobs \<langle>y,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> = [[yEvent]\<^sub>E]\<close> ttWF_xs_x ttWF_ys_y flt2_xs_x flt2_ys_y flt2cttobs_is_TT3_trace prirelRef prirelRef_extend_both_events_maximal_ttWF)
      next
        case False
        then show ?thesis
          proof (cases "xA")
            ...
            then show ?thesis
              by (metis \<open>flt2cttobs \<langle>x,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> = [[yEvent]\<^sub>E]\<close> \<open>flt2cttobs \<langle>y,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> = [[yEvent]\<^sub>E]\<close> append_Nil assms(8) ttWF_xs_x ttWF_ys_y flt2_xs_x flt2_ys_y prirelRef prirelRef_extend_both_events_non_maximal_ttWF)
...
lemma
  assumes "prirel p fl Y" "FL1 fl\<^sub>0" "FLTick0 Tick fl\<^sub>0"
          ...
          "fl2ctt fl\<^sub>0 \<subseteq> P"
          "flt2cttobs Y \<in> P"
          "ar = flt2cttobs fl" "flt2goodTock fl" "TTwf P"
        shows "\<exists>zr. prirelRef p (flt2cttobs fl) zr [] P \<and> zr \<in> P"
  using pp 
...
lemma TT1c_prefix_is_in:
  assumes "TT1c P" "s @ t \<in> P"
  shows "s \<in> P"
  using assms unfolding TT1c_def 
...
proof (induct p xs ys arbitrary:s x y rule:prirel.induct)
  ...
  next
    ...
    then show ?thesis using 1 
      ...
      apply (cases x, safe)
      apply force
      by (metis (no_types, lifting) "1.prems"(1) acceptance.simps(3) bullet_left_zero2 flt2cttobs.simps(1) prirelRef.simps(2) prirelacc_eq_prirelref_via_flt2cttobs)
  qed
next
case (2 p A Z zz)
  then show ?case
...
next
  ...
  from 4 have "prirelRef p (flt2cttobs ((A #\<^sub>\<F>\<^sub>\<L> aa) &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs ((Z #\<^sub>\<F>\<^sub>\<L> zz) &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) s P"
    by auto
  then have "prirelRef p (flt2cttobs (\<langle>A,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> (aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>))) (flt2cttobs (\<langle>Z,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> (zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>))) s P"
    by auto
  then have "prirelRef p (flt2cttobs (\<langle>A,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2cttobs(aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs (\<langle>Z,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2cttobs(zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) s P"
  proof -
    have "flt2cttobs (\<langle>A,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> (aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) = flt2cttobs (\<langle>A,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2cttobs(aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
              "flt2cttobs (\<langle>Z,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> (zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) = flt2cttobs (\<langle>Z,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2cttobs(zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      using flt2goodTocks by auto
    then show ?thesis
      using \<open>prirelRef p (flt2cttobs (\<langle>A,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> (aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>))) (flt2cttobs (\<langle>Z,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> (zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>))) s P\<close> by auto 
  qed

  then have pp:"prirelRef p (flt2cttobs (\<langle>(Aa,a)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2cttobs(aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs (\<langle>(Zz,z)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2cttobs(zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) s P"
    using A Z by auto

  (* Aim is to deduce prirelRef p (flt2cttobs (aa &\<^sub>\<F>\<^sub>\<L> \<langle>?x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs (zz &\<^sub>\<F>\<^sub>\<L> \<langle>?y\<rangle>\<^sub>\<F>\<^sub>\<L>)) ?s P *)
  then show ?case
  proof (cases a)
    case (Event x1)
    then have p1:"prirelRef p ([[Event x1]\<^sub>E] @ flt2cttobs(aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs (\<langle>(Zz,z)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2cttobs(zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) s P"
      using pp A by auto
    then have "flt2cttobs (\<langle>(Zz,z)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = [[Event x1]\<^sub>E]"
      using Z flt2goodTocks by (cases z, auto)
    
    then have "prirelRef p ([[Event x1]\<^sub>E] @ flt2cttobs(aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) ([[Event x1]\<^sub>E] @ flt2cttobs(zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) s P"
      using p1 by auto
    then have "prirelRef p (flt2cttobs (aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs (zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) (s@[[Event x1]\<^sub>E]) P"
      by auto
  ...
  next
    ...
    then have p1:"prirelRef p ([[{x. x \<notin>\<^sub>\<F>\<^sub>\<L> Aa}]\<^sub>R,[Tock]\<^sub>E] @ flt2cttobs(aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs (\<langle>(Zz,z)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2cttobs(zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) s P"
      using pp A Tock by (cases Aa, auto)
    then have "flt2cttobs (\<langle>(Zz,z)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = [[{x. x \<notin>\<^sub>\<F>\<^sub>\<L> Zz}]\<^sub>R,[Tock]\<^sub>E]"
      using Z flt2goodTocks by (cases z, auto)
    
    then have "prirelRef p ([[{x. x \<notin>\<^sub>\<F>\<^sub>\<L> Aa}]\<^sub>R,[Tock]\<^sub>E] @ flt2cttobs(aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) ([[{x. x \<notin>\<^sub>\<F>\<^sub>\<L> Zz}]\<^sub>R,[Tock]\<^sub>E] @ flt2cttobs(zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) s P"
      using p1 by auto
    then have "prirelRef p (flt2cttobs (aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs (zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) (s@[[{x. x \<notin>\<^sub>\<F>\<^sub>\<L> Zz}]\<^sub>R,[Tock]\<^sub>E]) P"
      by auto
  ...
  next
    case Tick
    then have p1:"prirelRef p ([[Tick]\<^sub>E] @ flt2cttobs(aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs (\<langle>(Zz,z)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2cttobs(zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) s P"
      using pp A by auto
    then have "flt2cttobs (\<langle>(Zz,z)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = [[Tick]\<^sub>E]"
      using Z flt2goodTocks by (cases z, auto)
    
    then have "prirelRef p ([[Tick]\<^sub>E] @ flt2cttobs(aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) ([[Tick]\<^sub>E] @ flt2cttobs(zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) s P"
      using p1 by auto
    then have "prirelRef p (flt2cttobs (aa &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)) (flt2cttobs (zz &\<^sub>\<F>\<^sub>\<L> \<langle>y\<rangle>\<^sub>\<F>\<^sub>\<L>)) (s@[[Tick]\<^sub>E]) P"
      by auto
...
proof (induct p xs ys s P arbitrary:zr rule:prirelRef.induct, auto simp add:TT3_trace_cons_imp_cons)
  ...
  then show "\<exists>fl. prirel pa fl zra \<and> flt2cttobs fl = [] \<and> flt2goodTock fl"
...
next
  ...
  from assm1 assm2 have "zra = \<langle>[{x. x \<notin> S}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>"
    ...
    by (case_tac b, auto)
  then show "\<exists>fl. prirel pa fl zra \<and> flt2cttobs fl = [[prirelref pa S]\<^sub>R] \<and> flt2goodTock fl"
...
next
  ...
  fix aa S zz sa Q
  fix zra::"'a cttevent fltrace"
  assume hyp:"(\<And>zr. flt2cttobs zr = zz \<Longrightarrow> flt2goodTock zr \<Longrightarrow> flt2goodAcceptance zr pa \<Longrightarrow> \<exists>fl. prirel pa fl zr \<and> flt2cttobs fl = aa \<and> flt2goodTock fl)"
  ...
  then obtain tt where tt:"flt2cttobs tt = zz \<and> flt2goodTock tt \<and> flt2goodAcceptance tt pa \<and> zra = \<langle>([{x. x \<notin> S}]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> tt"
    ...
    apply (case_tac x1a, auto, case_tac b, auto, case_tac a, auto)
     apply metis
    by (case_tac b, auto)
  then obtain flaa where flaa:"prirel pa flaa tt \<and> flt2cttobs flaa = aa \<and> flt2goodTock flaa"
    using hyp assm8 by blast
    
  show "\<exists>fl. prirel pa fl zra \<and> flt2cttobs fl = [prirelref pa S]\<^sub>R # [Tock]\<^sub>E # aa \<and> flt2goodTock fl"
  proof -
    ...
    then show ?thesis using flt2cttobs_fla_flaa
...
next
  ...
  assume hyp:"(\<And>zr. ttWF zz \<Longrightarrow>
              flt2cttobs zr = zz \<Longrightarrow> flt2goodTock zr \<Longrightarrow> flt2goodAcceptance zr pa \<Longrightarrow> \<exists>fl. prirel pa fl zr \<and> flt2cttobs fl = aa \<and> flt2goodTock fl)"
  ...
  have zz_is_ttWF:"ttWF zz"
    using assm2
    by (metis ttWF.simps(1) ttWF.simps(4) ttWF.simps(8) cttevent.exhaust neq_Nil_conv no_Tock)
  ...
  by auto
  then obtain fl where fl:"prirel pa fl tt \<and> flt2cttobs fl = aa \<and> flt2goodTock fl"
    using assm2 assm4 hyp zz_is_ttWF
    by blast
  show "\<exists>fl. prirel pa fl zra \<and> flt2cttobs fl = [e\<^sub>2]\<^sub>E # aa \<and> flt2goodTock fl"
  proof -
...
next
  ...
  assume hyp:"(\<And>zr. ttWF zz \<Longrightarrow> flt2cttobs zr = zz \<Longrightarrow> flt2goodTock zr \<Longrightarrow> flt2goodAcceptance zr pa \<Longrightarrow> \<exists>fl. prirel pa fl zr \<and> flt2cttobs fl = aa \<and> flt2goodTock fl)"
  assume assm1:"TT3_trace ([e\<^sub>2]\<^sub>E # zz)"
  assume assm2:"ttWF ([e\<^sub>2]\<^sub>E # zz)"
  then have no_Tock:"e\<^sub>2 \<noteq> Tock"
    using ttWF.simps(6) by blast
  ...
  from assm3 obtain tt aE where tt:"flt2cttobs tt = zz \<and> flt2goodTock tt 
    ...
    apply (case_tac x, auto)
    apply (case_tac x1a, auto)
    apply (metis acceptance_event acceptance_set ttWF.simps(6) cttobs.inject(1) list.inject zz_is_ttWF)
      ...
      show ?thesis 
        proof (cases "maximal(pa,e\<^sub>2)")
          ...
          proof -
            ...
            then obtain ffa :: "'a cttevent fltrace \<Rightarrow> 'a cttevent fltrace" where
              f2: "prirel pa (ffa ff) ff \<and> flt2cttobs (ffa ff) = aa \<and> flt2goodTock (ffa ff)"
...
next
  ...
  proof -
    ...
    have tocks:"Tock \<in>\<^sub>\<F>\<^sub>\<L> [{x. x \<notin> prirelref pa S}]\<^sub>\<F>\<^sub>\<L>"
               "Tock \<in>\<^sub>\<F>\<^sub>\<L> [{x. x \<notin> S}]\<^sub>\<F>\<^sub>\<L>"
      apply (metis TT3_trace.simps(3) flt2cttobs_is_TT3_trace \<open>prirelRef pa ([prirelref pa S]\<^sub>R # [Tock]\<^sub>E # flt2cttobs fl) ([S]\<^sub>R # [Tock]\<^sub>E # flt2cttobs zr) sa Q\<close> amember.simps(2) mem_Collect_eq xp)
      by (simp_all add:  assm4)
...
next
  ...
  assume assm0:"(ttWF zz \<Longrightarrow> \<exists>fl zr. prirel pa fl zr \<and> flt2cttobs fl = aa \<and> flt2cttobs zr = zz)"
...
proof -
  ...
  then obtain Z where fl:"ys = flt2cttobs Z \<and> flt2goodTock Z  \<and>
        (\<exists>fl\<^sub>0. FLTick0 Tick fl\<^sub>0 \<and> FL1 fl\<^sub>0 \<and> fl2ctt fl\<^sub>0 \<subseteq> P \<and> Z \<in> fl\<^sub>0) \<and> flt2cttobs Z \<in> P \<and> flt2goodAcceptance Z p \<and> flt2goodTock Z 
        \<and> ttWF ys \<and> prirelRef p xs ys [] P"
    unfolding fl2ctt_def using assms TTwf_1c_3_imp_flt2cttobs_FL1_mod by blast
  then have "\<exists>fl\<^sub>0. FLTick0 Tick fl\<^sub>0 \<and> FL1 fl\<^sub>0 \<and> fl2ctt fl\<^sub>0 \<subseteq> P \<and> Z \<in> fl\<^sub>0 \<and> flt2cttobs Z \<in> P \<and> flt2cttobs Z = ys \<and> flt2goodAcceptance Z p 
        \<and> flt2goodTock Z 
  ...
  then have "\<exists>fl\<^sub>0. FLTick0 Tick fl\<^sub>0 \<and> FL1 fl\<^sub>0 \<and> fl2ctt fl\<^sub>0 \<subseteq> P \<and> Z \<in> fl\<^sub>0 \<and> flt2cttobs Z \<in> P \<and> flt2cttobs Z = ys \<and> flt2goodAcceptance Z p 
        \<and> flt2goodTock Z 
        \<and> TT3_trace ys \<and> ttWF ys \<and> prirelRef p xs ys [] P
        \<and> (\<exists>fl. prirel p fl Z \<and> flt2cttobs fl = xs \<and> flt2goodTock fl)"
    using pp20 assms
    by metis
  then have "\<exists>fl\<^sub>0. FLTick0 Tick fl\<^sub>0 \<and> FL1 fl\<^sub>0 \<and> fl2ctt fl\<^sub>0 \<subseteq> P \<and> Z \<in> fl\<^sub>0 \<and> flt2cttobs Z \<in> P \<and> flt2goodTock Z 
        \<and> (\<exists>fl. prirel p fl Z \<and> flt2cttobs fl = xs \<and> flt2goodTock fl)"
    by blast
...
proof -
  have "fl2ctt(pri p (ctt2fl P)) = {flt2cttobs fl|fl. fl \<in> (pri p (ctt2fl P)) \<and> flt2goodTock fl} \<union> {[]}"
    ...
    by (simp add: fl2ctt_FL0_FL1_flt2goodTock TT0_healthy TT1c_healthy TickTock_FL.FL0_ctt2fl FL1_ctt2fl pri_FL0 pri_FL1)
  ...
  also have "... = {ar|ar zr. prirelRef p ar zr [] P \<and> zr \<in> P}"
    using assms apply auto
    using TT0_TT1c_empty prirelRef.simps(1) apply blast
...
lemma flt2cttobs_exists_flt2goodTock_for_ttWF_TT3_trace:
  assumes "ttWF fl" "TT3_trace fl"
...
next
  fix e \<sigma>
  assume hyp:"(TT3_trace \<sigma> \<Longrightarrow> \<exists>zr. flt2cttobs zr = \<sigma> \<and> flt2goodTock zr)"
  assume assm1:"ttWF \<sigma>"
  assume assm2:"TT3_trace ([Event e]\<^sub>E # \<sigma>)"
