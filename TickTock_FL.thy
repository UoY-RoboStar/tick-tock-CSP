...
fun flt2ttobs :: "('e ttevent) fltrace \<Rightarrow> ('e ttobs) list" where
"flt2ttobs \<langle>A\<rangle>\<^sub>\<F>\<^sub>\<L> = (if A = \<bullet> then [] else [[{z. z \<notin>\<^sub>\<F>\<^sub>\<L> A}]\<^sub>R])" |
"flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl) = (if event(A) = Tock then
                             (if acceptance(A) \<noteq> \<bullet> then
                              ([{z. z \<notin>\<^sub>\<F>\<^sub>\<L> acceptance(A)}]\<^sub>R # [Tock]\<^sub>E # (flt2ttobs fl))
                              else []) 
                          else ([event A]\<^sub>E # flt2ttobs fl))" 
...
definition TTRMax :: "('a ttobs) list set \<Rightarrow> bool" where
...
definition TTickAll :: "('a ttobs) list set \<Rightarrow> bool" where
...
lemma flt2ttobs_is_ttWF:
  ...
  shows "ttWF (flt2ttobs fltrace)"
  using assms
  apply (induct fltrace rule:flt2ttobs.induct, auto)
  apply (case_tac A, auto, case_tac a, auto, case_tac b, auto)
  by (metis ttWF.simps(4) ttWF2.simps(1) ttWF2.simps(23) ttWF2_ttWF ttevent.exhaust flt2ttobs.simps(1))

definition fl2tt :: "('e ttevent) fltrace set \<Rightarrow> ('e ttobs) list set" where
"fl2tt P = {flt2ttobs fl|fl. fl \<in> P}"

lemma fl2tt_univ_disj:
  "fl2tt (\<Union> P) = \<Union>{fl2tt fl| fl. fl \<in> P}"
  unfolding fl2tt_def by auto

definition tt2fl :: "('e ttobs) list set \<Rightarrow> ('e ttevent) fltrace set" where
"tt2fl P = \<Union>{fl. FLTick0 Tick fl \<and> FL1 fl \<and> (fl2tt fl) \<subseteq> P}"

lemma tt2fl_mono:
  assumes "P \<subseteq> Q"
  shows "tt2fl(P) \<subseteq> tt2fl(Q)"
  using assms unfolding tt2fl_def by auto
...
lemma last_flt2ttobs_eq_ref_imp_last:
  assumes "flt2ttobs (xs) \<noteq> []" "List.last(flt2ttobs (xs)) = Ref r" 
...
proof -
  ...
  *)
  using assms apply (induct xs rule:flt2ttobs.induct, auto)
   apply (case_tac A, auto)
  apply (case_tac A, auto)
   apply (case_tac b, auto)
     apply (metis ttobs.distinct(1) flt2ttobs.simps(1))
    apply (smt List.last.simps acceptance.distinct(1) amember.elims(2) ttobs.distinct(1) flt2ttobs.simps(1) list.discI)
   apply (metis ttobs.distinct(1) flt2ttobs.simps(1))
   apply (case_tac b, auto)
    apply (metis ttobs.distinct(1) flt2ttobs.simps(1))
  apply (case_tac fl, auto)
  apply (metis (mono_tags, lifting) Collect_cong ttobs.distinct(1) flt2ttobs.simps(1))
  by (smt ttobs.distinct(1) flt2ttobs.simps(2) last_ConsR list.discI)
  

lemma concat_ref_tock_imp_tock_in_last_of_flt2ttobs:
  assumes "ys @ [[x1]\<^sub>R] @ [[Tock]\<^sub>E] \<in> P" "TTwf P" "TT3 P"
          "ys @ [[x1]\<^sub>R] @ [[Tock]\<^sub>E] = flt2ttobs fl @ [[Tock]\<^sub>E]"
    shows "Tock \<in>\<^sub>\<F>\<^sub>\<L> last fl"
proof -
  have a:"ys @ [[x1]\<^sub>R] = flt2ttobs fl" "fl \<noteq> \<langle>\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>"
    using assms 
    by (induct fl, auto)
  then have b:"flt2ttobs fl @ [[Tock]\<^sub>E] \<in> P"
    using assms apply auto
    by (metis assms(1) assms(4))
  then have "flt2ttobs fl \<noteq> []"
    using a(1) by auto
  then show ?thesis 
    using a assms 
    ...
    next
      ...
      then obtain r where r:"List.last(flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> x)) = [r]\<^sub>R"
        using 2
        by (metis last_snoc)
      ...
      then have last_eq: "last (xs &\<^sub>\<F>\<^sub>\<L> x) = [{x. x \<notin> r}]\<^sub>\<F>\<^sub>\<L>"
        using r
        using "2.prems"(3)
        using "2.prems"(1) last_flt2ttobs_eq_ref_imp_last by fastforce
...
lemma
  ...
  shows "flt2ttobs fl \<noteq> []"
  nitpick

lemma
  assumes "last xs = \<bullet>" "flt2ttobs(xs) \<noteq> []" "flt2ttobs(ys) \<noteq> []"
  shows "flt2ttobs (xs @\<^sub>\<F>\<^sub>\<L> ys) = flt2ttobs(xs) @ flt2ttobs(ys)"
  nitpick

lemma
  assumes "flt2ttobs (butlast fl) \<noteq> []" "Tock \<in>\<^sub>\<F>\<^sub>\<L> last fl"
  shows "flt2ttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>(last fl,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs(butlast fl) @ flt2ttobs(\<langle>(last fl,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
  using assms apply (induct fl, auto)
  apply (case_tac x1a, auto, case_tac b, auto, case_tac a, auto)
*)

lemma flt2ttobs_last_tock:
  assumes "Tock \<in>\<^sub>\<F>\<^sub>\<L> last fl" "flt2goodTock fl"
  shows "flt2ttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>(last fl,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs fl @ [[Tock]\<^sub>E]"
  using assms
  by (induct fl rule:flt2ttobs.induct, auto)

lemma flt2ttobs_butlast_cons_eq_list_cons:
  assumes "flt2goodTock fl"
  shows "flt2ttobs (butlast fl &\<^sub>\<F>\<^sub>\<L> \<langle>y,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs(butlast fl) @ flt2ttobs(\<langle>y,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
  using assms by (induct fl, auto)

lemma flt2ttobs_acceptance_cons_eq_list_cons:
  assumes "last fl = \<bullet>" "flt2goodTock fl"
  shows "flt2ttobs (fl &\<^sub>\<F>\<^sub>\<L> \<langle>y,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs(fl) @ flt2ttobs(\<langle>y,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
  using assms
  by (metis bullet_right_zero2 butlast_last_cons2_FL flt2ttobs_butlast_cons_eq_list_cons)

(* FIXME: Horrendous proof below *)
lemma flt2ttobs_last_non_tock:
  assumes "e \<noteq> Tock" "e \<in>\<^sub>\<F>\<^sub>\<L> last fl \<or> last fl = \<bullet>" "flt2goodTock fl" 
          "List.last(flt2ttobs fl) = [f]\<^sub>E"
  shows "flt2ttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>(last fl,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs fl @ [[e]\<^sub>E]"
  using assms
  apply (induct fl rule:flt2ttobs.induct, auto)
  apply (case_tac A, auto)
          apply (case_tac A, auto)
         apply (case_tac A, auto, case_tac b, auto)
  apply (case_tac a, auto)
           apply (case_tac "flt2ttobs fla = []", auto)
  apply (metis (no_types, lifting) acceptance_event flt2ttobs.simps(1) flt2ttobs.simps(2) flt2goodTock.elims(2) fltrace_concat.simps(3) list.discI)
  apply (metis (no_types, lifting) List.butlast.simps(2) acceptance_event append_eq_append_conv2 butlast_snoc flt2ttobs.simps(1) flt2ttobs.simps(2) flt2goodTock.elims(2) fltrace_concat.simps(3) list.discI)
  apply (metis (no_types, lifting) List.butlast.simps(2) acceptance_event append_eq_append_conv2 butlast_snoc flt2ttobs.simps(1) flt2ttobs.simps(2) flt2goodTock.elims(2) fltrace_concat.simps(3) list.discI)
  apply (metis (no_types, lifting) List.butlast.simps(2) acceptance_event append_eq_append_conv2 butlast_snoc flt2ttobs.simps(1) flt2ttobs.simps(2) flt2goodTock.elims(2) fltrace_concat.simps(3) list.discI)
         apply (case_tac A, auto, case_tac b, auto)
  apply (metis (no_types, lifting) List.butlast.simps(2) acceptance_event append_eq_append_conv2 butlast_snoc flt2ttobs.simps(1) flt2ttobs.simps(2) flt2goodTock.elims(2) fltrace_concat.simps(3) list.discI)
  ...
  apply (metis (no_types, lifting) List.butlast.simps(2) acceptance_event append_eq_append_conv2 butlast_snoc flt2ttobs.simps(1) flt2ttobs.simps(2) flt2goodTock.elims(2) fltrace_concat.simps(3) list.discI)
  apply (metis (no_types, lifting) List.butlast.simps(2) acceptance_event append_eq_append_conv2 butlast_snoc flt2ttobs.simps(1) flt2ttobs.simps(2) flt2goodTock.elims(2) fltrace_concat.simps(3) list.discI)
  by (metis (no_types, lifting) List.butlast.simps(2) acceptance_event append_eq_append_conv2 butlast_snoc flt2ttobs.simps(1) flt2ttobs.simps(2) flt2goodTock.elims(2) fltrace_concat.simps(3) list.discI)

lemma flt2ttobs_fl_acceptance:
  assumes "flt2goodTock fl" 
          "List.last(flt2ttobs fl) = [f]\<^sub>E" "flt2ttobs fl \<noteq> []"
  shows "flt2ttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>[{x. x \<notin> r}]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs fl @ [[r]\<^sub>R]"
  using assms
  apply (induct fl rule:flt2ttobs.induct, auto)
    apply (case_tac A, auto)
   apply (case_tac A, auto, case_tac b, auto)
     by (case_tac "flt2ttobs fla = []", auto, case_tac fla, auto, meson list.discI)+

lemma funFLTTockl_last_fl_not_bullet_dist_list_cons:
  assumes "last fl \<noteq> \<bullet>"
  shows "flt2ttobs(fl) = flt2ttobs(butlast fl) @ flt2ttobs(\<langle>last fl\<rangle>\<^sub>\<F>\<^sub>\<L>)"
  nitpick
  using assms apply (induct fl, auto)
  oops
...
lemma strong_less_eq_fltrace_imp_flt2ttobs_tt:
  assumes "s \<le>\<^sub>\<F>\<^sub>\<L> t"
  shows "flt2ttobs s \<le>\<^sub>C flt2ttobs t"
...
lemma flt2ttobs_last_fl_not_bullet_dist_list_cons:
...
lemma event_not_in_set_of_flt2ttobs_imp_not_in_events:
  assumes "[e]\<^sub>E \<notin> set ys" "flt2ttobs zs = ys" "flt2goodTock zs"
  shows "e \<notin> events zs"
  using assms apply (induct zs arbitrary: ys e rule:flt2ttobs.induct, auto) 
...
next
  ...
  next
    ...
    then have ys_y_fl:"\<exists>fl. ys @ [y] = flt2ttobs fl \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)"
      using yys by auto
    then have ys_y_x: "\<exists>fl. ys @ [y] @ [x] = flt2ttobs fl @ [x] \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)"
      by auto

    then show ?case
    proof (cases y)
      case r1:(Ref r1)
      then have exp:"\<exists>fl. ys @ [[r1]\<^sub>R] @ [x] = flt2ttobs fl @ [x] \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)"
      ...
      next
        ...
        next
          ...
          then obtain fl where fl:"ys @ [Ref r1] = flt2ttobs fl \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and>  {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)"
          ...
          then have "ys @ [[r1]\<^sub>R] @ [[Tock]\<^sub>E] = flt2ttobs fl @ [[Tock]\<^sub>E] \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)"
            by (simp add: fl)
          then have "ys @ [[r1]\<^sub>R] @ [[Tock]\<^sub>E] = flt2ttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>(last fl,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)"
          ...
          have "(ys @ [[r1]\<^sub>R] @ [[Tock]\<^sub>E] = flt2ttobs(fl @\<^sub>\<F>\<^sub>\<L> \<langle>(last fl,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> flt2goodTock fl
            ...
            using \<open>ys @ [[r1]\<^sub>R] @ [[Tock]\<^sub>E] = flt2ttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>(Finite_Linear_Model.last fl,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)\<close> tock_in_last_fl by blast
          then have 
               "ys @ [[r1]\<^sub>R] @ [[Tock]\<^sub>E] = flt2ttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>(last fl,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> flt2goodTock fl  
            ...
            by (metis (mono_tags, lifting) Collect_cong \<open>ys @ [[r1]\<^sub>R] @ [[Tock]\<^sub>E] = flt2ttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>(Finite_Linear_Model.last fl,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)\<close> fl)
          then have 
               "ys @ [[r1]\<^sub>R] @ [[Tock]\<^sub>E] = flt2ttobs (fl @\<^sub>\<F>\<^sub>\<L> \<langle>(last fl,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> flt2goodTock fl  
          ...
          then have 
               "\<exists>fl. ys @ [[r1]\<^sub>R] @ [[Tock]\<^sub>E] = flt2ttobs fl \<and> flt2goodTock fl 
    ...
    next
      case e1:(ObsEvent e1)
      text \<open>Then from the hypothesis we have the case:\<close>
      then obtain fl where fl:"ys @ [[e1]\<^sub>E] = flt2ttobs fl \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)"
        using ys_y_fl by blast
      then have ys_e1_x:"ys @ [[e1]\<^sub>E] @ [x] = flt2ttobs fl @ [x] \<and> flt2goodTock fl \<and> (\<exists>x. FLTick0 Tick x \<and> FL1 x \<and> {flt2ttobs fl |fl. fl \<in> x} \<subseteq> P \<and> fl \<in> x)"
        by auto
...
lemma fl2tt_mono:
  ...
  using assms unfolding fl2tt_def by auto

lemma
  assumes TTwf_healthy: "TTwf P" 
  shows "\<forall>x. x \<in> tt2fl(P) \<longrightarrow> tickWF Tick x"
  using assms unfolding tt2fl_def apply auto
  oops

lemma flt2ttobs_flt2goodTock_less_eq_exists:
  assumes "flt2ttobs fl \<noteq> []"
  shows "\<exists>fla. flt2ttobs fl = flt2ttobs fla \<and> fla \<le> fl \<and> flt2goodTock fla"
  using assms
...
next
  case (2 A fl)
  then show ?case
  proof (cases "flt2ttobs fl \<noteq> []")
    case flt2ttobs_not_Nil:True
    then show ?thesis
      proof (cases "acceptance(A) \<noteq> \<bullet>")
      case True
        then have "\<exists>flaa. flt2ttobs fl = flt2ttobs flaa \<and> flaa \<le> fl \<and> flt2goodTock flaa"
          using flt2ttobs_not_Nil 2 by auto
        then show ?thesis using 2 True
          by (metis flt2ttobs.simps(2) flt2goodTock.simps(2) less_eq_fltrace.simps(3) order_refl)
      next
        case False
        then show ?thesis 
          using flt2ttobs_not_Nil 2 apply auto
          by (metis flt2ttobs.simps(2) flt2goodTock.simps(2) less_eq_fltrace.simps(3) order_refl)
      qed
  next
...
lemma flt2ttobs_FL1_exists_flt2goodTock:
  assumes "flt2ttobs fl \<noteq> []" "fl \<in> P" "FL1 P"
  shows "\<exists>fla. flt2ttobs fl = flt2ttobs fla \<and> fla \<in> P \<and> flt2goodTock fla"
  using assms
  by (meson FL1_def flt2ttobs_flt2goodTock_less_eq_exists)

lemma fl2tt_FL0_FL1_flt2goodTock_non_bullet:
  assumes "FL0 P" "FL1 P"
  shows "fl2tt P = {flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl \<and> fl \<noteq> \<langle>\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>} \<union> {[]}"
  using assms unfolding fl2tt_def apply auto
  using flt2ttobs_FL1_exists_flt2goodTock
  apply (metis Finite_Linear_Model.butlast.simps(1) Finite_Linear_Model.last.simps(1) acceptance.distinct(1) append_self_conv2 flt2ttobs_last_fl_not_bullet_dist_list_cons flt2goodTock.simps(1))
  by (rule exI[where x="\<langle>\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>"], auto)

lemma fl2tt_FL0_FL1_flt2goodTock:
  assumes "FL0 P" "FL1 P"
  shows "fl2tt P = {flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl} \<union> {[]}"
  using assms unfolding fl2tt_def apply auto
  using flt2ttobs_FL1_exists_flt2goodTock
  apply (metis)
  by (rule exI[where x="\<langle>\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>"], auto)
...
lemma FLTick0_Tick_FL1_concat_ref_Tick_in:
  assumes "FLTick0 Tick P" "t @ [[X]\<^sub>R] = flt2ttobs fl" "fl \<in> P" "flt2goodTock fl"
  shows "Tick \<in> X"
...
next
  ...
  proof (cases "last xs = \<bullet>")
    case True
    then have "t @ [[X]\<^sub>R] = flt2ttobs (xs) @ flt2ttobs(\<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      using 2
      by (metis Finite_Linear_Model.last.simps(1) acceptance.distinct(1) flt2ttobs_last_fl_not_bullet_dist_list_cons last_bullet_butlast_last last_bullet_then_last_cons last_flt2ttobs_eq_ref_imp_last snoc_eq_iff_butlast)
    then have X_x:"[[X]\<^sub>R] = flt2ttobs(\<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      by (metis "2.prems"(2) Nil_is_append_conv True acceptance.distinct(1) bullet_right_zero2 flt2ttobs.simps(1) last_flt2ttobs_eq_ref_imp_last last_snoc)
    then show ?thesis
    proof (cases x)
      case acnil
  ...
  next
    case False
    then have "t @ [[X]\<^sub>R] = flt2ttobs (xs)"
      using 2
      by (simp add: concat_FL_last_not_bullet_absorb)
    then have X_x:"[[X]\<^sub>R] = flt2ttobs(\<langle>last xs\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      using "2.prems"(4) False concat_FL_last_not_bullet_absorb flt2ttobs_last_fl_not_bullet_dist_list_cons by fastforce
    then show ?thesis
      by (metis "2.hyps" "2.prems"(3) "2.prems"(4) False \<open>t @ [[X]\<^sub>R] = flt2ttobs xs\<close> assms(1) concat_FL_last_not_bullet_absorb)
  qed
next
  case (3 x xs)
  then show ?case
    proof (cases "last xs = \<bullet>")
      case True
      then show ?thesis
        by (metis "3.prems"(2) List.last.simps acceptance.distinct(1) append_is_Nil_conv last_appendR last_cons_bullet_iff last_flt2ttobs_eq_ref_imp_last list.simps(3))
    next
      case False
      then have "t @ [[X]\<^sub>R] = flt2ttobs (xs)"
      using 3
      by (simp add: concat_FL_last_not_bullet_absorb)
    then have X_x:"[[X]\<^sub>R] = flt2ttobs(\<langle>last xs\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      using "3.prems"(4) False concat_FL_last_not_bullet_absorb flt2ttobs_last_fl_not_bullet_dist_list_cons by fastforce
    then show ?thesis
      by (metis "3.hyps" "3.prems"(4) "3.prems"(3) \<open>t @ [[X]\<^sub>R] = flt2ttobs xs\<close> assms(1) concat_FL_last_not_bullet_absorb flt2ttobs.simps(1) list.distinct(1))
    qed
  qed
...
lemma TTick_fl2tt:
  ...
  shows "TTick (fl2tt P)"
proof -
  have "TTick (fl2tt P) = TTick({flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl} \<union> {[]})"
    using assms
    by (simp add: fl2tt_FL0_FL1_flt2goodTock)
  also have "... = TTick({flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl})"
    using TTick_dist_empty_trace by auto
  also have "... = True"
    unfolding TTick_def fl2tt_def apply auto
    using assms FLTick0_Tick_FL1_concat_ref_Tick_in by metis
  finally show ?thesis by auto
qed

lemma tickWF_imp_TTickTrace_flt2ttobs:
  assumes "tickWF Tick fl"
  shows "TTickTrace (flt2ttobs fl)"
  using assms apply (induct fl rule:flt2ttobs.induct, auto)
   apply (case_tac A, auto, case_tac a, auto, case_tac b, auto)
  by (case_tac A, auto, case_tac b, auto)

lemma TTickAll_fl2tt:
  assumes "FL0 P" "FL1 P" "FLTick0 Tick P"
  shows "TTickAll (fl2tt P)"
proof -
  have "TTickAll (fl2tt P) = TTickAll({flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl} \<union> {[]})"
    using assms
    by (simp add: fl2tt_FL0_FL1_flt2goodTock)
  also have "... = TTickAll({flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl})"
    using TTickAll_dist_empty_trace by auto
  also have "... = True"
    unfolding TTickAll_def fl2tt_def apply auto
    using assms tickWF_imp_TTickTrace_flt2ttobs
...
lemma prefix_tt2fl_FL1:
  assumes "t \<in> tt2fl P"
          "s \<le> t"
    shows "s \<in> tt2fl P"
  using assms unfolding tt2fl_def apply auto
...
lemma FL1_tt2fl:
  ...
  using prefix_tt2fl_FL1 by blast

(*
lemma
  assumes "tickWF Tick fltrace"
  shows "TTwf (fl2tt xa)"
  using assms unfolding fl2tt_def TTwf_def apply auto
  sledgehammer
*)

lemma FLTick0_Tick_tt2fl:
  assumes "TTwf P"
  shows "FLTick0 Tick (tt2fl P)"
  using assms unfolding tt2fl_def FLTick0_def TTwf_def by auto
...
proof (cases "last \<beta> = \<bullet>")
  ...
  proof (cases "flt2goodTock \<beta>")
    case True
    then have "flt2ttobs (\<beta>) @ flt2ttobs (\<langle>A\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P"
      by (metis (no_types, lifting) Finite_Linear_Model.last.simps(1) acceptance.simps(3) amember.elims(2) assms(1) assms(2) butlast_last_FL flt2ttobs_last_fl_not_bullet_dist_list_cons flt2goodTock_extend_consFL_acceptance last_B_bullet last_bullet_butlast_last last_bullet_then_last_cons)
    then obtain R where R:"R = {x. x \<notin>\<^sub>\<F>\<^sub>\<L> A}"
      using assms(2) by force
    then have "a \<notin> R"
      using assms(2) by blast
    then have "flt2ttobs (\<beta>) @ [[R]\<^sub>R] \<in> P"
      using R by (metis \<open>flt2ttobs \<beta> @ flt2ttobs \<langle>A\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P\<close> amember.simps(1) assms(2) flt2ttobs.simps(1))
    then show ?thesis
    proof (cases a)
      case (Event x1)
      then have "flt2ttobs (\<beta>) @ [[a]\<^sub>E] \<in> P"
        using assms
        using TTM2a_def \<open>a \<notin> R\<close> \<open>flt2ttobs \<beta> @ [[R]\<^sub>R] \<in> P\<close> by blast
      then show ?thesis
        using Event True assms(2) flt2ttobs_acceptance_cons_eq_list_cons last_B_bullet by fastforce
    next
      case Tock
      then have flt2ttobs_bullet:"flt2ttobs (\<beta> &\<^sub>\<F>\<^sub>\<L> \<langle>(\<bullet>,a)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs (\<beta>)"
        by (simp add: True flt2ttobs_acceptance_cons_eq_list_cons last_B_bullet)
      then have flt2ttobs_R_tock:"flt2ttobs (\<beta>) @ [[R]\<^sub>R,[a]\<^sub>E] \<in> P"
        using Tock TTM2b_def \<open>a \<notin> R\<close> \<open>flt2ttobs \<beta> @ [[R]\<^sub>R] \<in> P\<close> assms(4) by blast
      then have c1:"flt2ttobs (\<beta> &\<^sub>\<F>\<^sub>\<L> \<langle>(A,a)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P"
    ...
    next
      case Tick
      then have "flt2ttobs (\<beta>) @ [[a]\<^sub>E] \<in> P"
        using TTM2a_def \<open>a \<notin> R\<close> \<open>flt2ttobs \<beta> @ [[R]\<^sub>R] \<in> P\<close> assms(3) by blast
      then show ?thesis
        using Tick True assms(2) flt2ttobs_acceptance_cons_eq_list_cons last_B_bullet by fastforce
    qed  
  next
    case False
    then show ?thesis
      using assms(1) flt2ttobs_not_flt2goodTock_imp_flt2ttobs_eq_consFL_any by fastforce
  qed
next
  case False
  then show ?thesis
...
case 1
  then show ?case by (metis (mono_tags, lifting) CollectI FL0_FL1_bullet_in_so flt2ttobs.simps(1) in_mono)
next
  case (2 z zs)
  then have "flt2ttobs (\<beta> &\<^sub>\<F>\<^sub>\<L> \<langle>A\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P"
    by blast
  then have flt2ttobs_B_Aa:
        "flt2ttobs (\<beta> &\<^sub>\<F>\<^sub>\<L> \<langle>(A,a)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P"
        "flt2ttobs (\<beta> &\<^sub>\<F>\<^sub>\<L> \<langle>(\<bullet>,a)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P"
    using flt2ttobs_for_FL2_imp "2.prems"(1) assms(2) assms(3) assms(4) by blast+
...
next
  ...
  then have flt2ttobs_B_Aa:
        "flt2ttobs (\<beta> &\<^sub>\<F>\<^sub>\<L> \<langle>(A,a)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P"
        "flt2ttobs (\<beta> &\<^sub>\<F>\<^sub>\<L> \<langle>(\<bullet>,a)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<in> P"
    using flt2ttobs_for_FL2_imp "3.prems"(1) assms(2) assms(3) assms(4) by blast+
...
lemma FL2_imp_TTM2a_part:
  assumes "FL2 P" "e \<notin> X" "e \<noteq> Tock" "\<rho> @ [[X]\<^sub>R] = flt2ttobs fl" "fl \<in> P" "flt2goodTock fl"
  shows "\<exists>fl. \<rho> @ [[e]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl"
  using assms 
...
next
  ...
  proof (cases "last xs = \<bullet>")
    ...
    proof (cases x)
      case acnil
      then show ?thesis
        by (metis "2.prems"(4) Nil_is_append_conv True acceptance.distinct(1) bullet_right_zero2 last_flt2ttobs_eq_ref_imp_last last_snoc not_Cons_self2)
    next
      case (acset x2)
      then have flt2goodTock_xs: "flt2goodTock (xs)"
        using 2
        by (metis True flt2ttobs_not_flt2goodTock_imp_flt2ttobs_eq_consFL_any last_cons_acceptance_not_bullet last_flt2ttobs_eq_ref_imp_last snoc_eq_iff_butlast)
      then have "flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs(xs) @ flt2ttobs(\<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
        using 2 True
        by (metis Finite_Linear_Model.last.simps(1) append_Nil2 bullet_right_zero2 flt2ttobs.simps(1) flt2ttobs_last_fl_not_bullet_dist_list_cons last_bullet_butlast_last last_bullet_then_last_cons)
      then have "\<rho> @ [[X]\<^sub>R] = flt2ttobs(xs) @ flt2ttobs(\<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
        using 2 by auto
      then have "\<rho> = flt2ttobs(xs)" "[[X]\<^sub>R] = flt2ttobs(\<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
         apply (metis List.last.simps Nil_is_append_conv True acceptance.distinct(1) append_Nil2 butlast_snoc flt2ttobs.simps(1) last_appendR last_flt2ttobs_eq_ref_imp_last)
        by (metis "2.prems"(4) Nil_is_append_conv True \<open>\<rho> @ [[X]\<^sub>R] = flt2ttobs xs @ flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> acceptance.distinct(1) bullet_right_zero2 flt2ttobs.simps(1) last_flt2ttobs_eq_ref_imp_last last_snoc)

      have \<rho>_e:"\<rho> @ [[e]\<^sub>E] = flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      proof -
        have "flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs(xs) @ flt2ttobs(\<langle>([x2]\<^sub>\<F>\<^sub>\<L>,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
          using assms
          by (metis "2.prems"(4) True \<open>\<rho> = flt2ttobs xs\<close> \<open>flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs xs @ flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> acset append_self_conv flt2ttobs_acceptance_cons_eq_list_cons flt2ttobs_not_flt2goodTock_imp_flt2ttobs_eq_consFL_any list.simps(3))
        also have "... = flt2ttobs(xs) @ [[e]\<^sub>E]"
          using "2.prems"(2) "2.prems"(3) \<open>[[X]\<^sub>R] = flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> acset by auto
        then show ?thesis
          by (simp add: \<open>\<rho> = flt2ttobs xs\<close> calculation)
      qed

      have "xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P"
        by (metis "2.prems"(2) "2.prems"(5) CollectI FL2_def List.last.simps \<open>[[X]\<^sub>R] = flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> acset assms(1) ttobs.inject(2) flt2ttobs.simps(1) list.simps(3))
      then have "\<rho> @ [[e]\<^sub>E] = flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P"
        using \<rho>_e
        by blast
      then have "\<rho> @ [[e]\<^sub>E] = flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P 
        ...
        by (metis CollectI \<open>[[X]\<^sub>R] = flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> acset ttobs.inject(2) flt2ttobs.simps(1) last_snoc not_Cons_self2)
...
next
  ...
  then show ?case
    proof (cases "last xs = \<bullet>")
      case True
      then show ?thesis
        by (metis "3.prems"(4) acceptance.distinct(1) last_cons_bullet_iff last_flt2ttobs_eq_ref_imp_last snoc_eq_iff_butlast)
    next
      case False
      then have flt2goodTock_xs:"flt2goodTock (xs)"
        using "3.prems"(6) concat_FL_last_not_bullet_absorb by fastforce
      have "[[X]\<^sub>R] = flt2ttobs(\<langle>last xs\<rangle>\<^sub>\<F>\<^sub>\<L>)"
        using False "3.prems"(4) "3.prems"(6) concat_FL_last_not_bullet_absorb flt2ttobs_last_fl_not_bullet_dist_list_cons by fastforce
      have "xs \<in> P"
        using 3 False
      ...
      then have xs_e:"butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P"
        by (metis "3.prems"(2) CollectI FL2_def False List.last.simps \<open>[[X]\<^sub>R] = flt2ttobs \<langle>Finite_Linear_Model.last xs\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> assms(1) ttobs.inject(2) flt2ttobs.simps(1))
      then have "\<rho> @ [[e]\<^sub>E] = flt2ttobs(butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
        by (metis (no_types, lifting) "3.prems"(2) "3.prems"(3) "3.prems"(4) "3.prems"(6) CollectI False acceptance_event butlast_snoc concat_FL_last_not_bullet_absorb ttobs.inject(2) flt2ttobs.simps(1) flt2ttobs.simps(2) flt2ttobs_butlast_cons_eq_list_cons flt2ttobs_last_fl_not_bullet_dist_list_cons last_snoc)
      then have "\<rho> @ [[e]\<^sub>E] = flt2ttobs(butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P"
        using xs_e by auto
      then have "\<rho> @ [[e]\<^sub>E] = flt2ttobs(butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) 
        ...
        by (metis (no_types, lifting) "3.prems"(2) "3.prems"(3) "3.prems"(4) CollectI False Finite_Linear_Model.last.simps(1) List.last.simps Nil_is_append_conv \<open>[[X]\<^sub>R] = flt2ttobs \<langle>Finite_Linear_Model.last xs\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> \<open>\<rho> @ [[e]\<^sub>E] = flt2ttobs (Finite_Linear_Model.butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(Finite_Linear_Model.last xs,e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)\<close> append_self_conv butlast_last_cons2_FL concat_FL_last_not_bullet_absorb ttobs.inject(2) flt2ttobs.simps(1) flt2ttobs_last_fl_not_bullet_dist_list_cons flt2ttobs_not_flt2goodTock_imp_flt2ttobs_eq_consFL_any flt2goodTock_consFL_imp last_flt2ttobs_eq_ref_imp_last last_snoc list.inject mem_Collect_eq not_Cons_self2 xs_e)
      then show ?thesis by blast
    qed
qed

lemma FL2_imp_TTM2b_part:
  assumes "FL2 P" "Tock \<notin> X" "\<rho> @ [[X]\<^sub>R] = flt2ttobs fl" "fl \<in> P" "flt2goodTock fl"
  shows "\<exists>fl. \<rho> @ [[X]\<^sub>R, [Tock]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl"
  using assms 
...
next
  ...
  proof (cases "last xs = \<bullet>")
    ...
    proof (cases x)
      case acnil
      then show ?thesis
        by (metis "2.prems"(3) True acceptance.distinct(1) bullet_right_zero2 last_flt2ttobs_eq_ref_imp_last snoc_eq_iff_butlast)
    next
      case (acset x2)
      then have flt2goodTock_xs: "flt2goodTock (xs)"
        using 2
      ...
      then have "\<rho> = flt2ttobs(xs)" "[[X]\<^sub>R] = flt2ttobs(\<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
         apply (metis List.last.simps Nil_is_append_conv True acceptance.distinct(1) append_Nil2 butlast_snoc flt2ttobs.simps(1) last_appendR last_flt2ttobs_eq_ref_imp_last)
        using \<open>\<rho> @ [[X]\<^sub>R] = flt2ttobs xs @ flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> acset by auto
 
      have \<rho>_e:"\<rho> @ [[{x. x \<notin> x2}]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      proof -
        have "flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) = flt2ttobs(xs) @ flt2ttobs(\<langle>([x2]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
          using assms True flt2ttobs_acceptance_cons_eq_list_cons flt2goodTock_xs by blast
        also have "... = flt2ttobs(xs) @ [[{x. x \<notin> x2}]\<^sub>R,[Tock]\<^sub>E]"
          using "2.prems"(2) "2.prems"(3) \<open>[[X]\<^sub>R] = flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> acset by auto
        then show ?thesis
          by (simp add: \<open>\<rho> = flt2ttobs xs\<close> calculation)
      qed
      ...
      have "xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P"
        by (metis "2.prems"(2) "2.prems"(4) CollectI FL2_def \<open>[[X]\<^sub>R] = flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> acceptance.simps(3) acset assms(1) ttobs.inject(2) flt2ttobs.simps(1) list.inject)
      then have "\<rho> @ [[{x. x \<notin> x2}]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P"
        using \<rho>_e
        by blast
      then have "\<rho> @ [[{x. x \<notin> x2}]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P 
                              \<and> flt2goodTock (xs &\<^sub>\<F>\<^sub>\<L> \<langle>([x2]\<^sub>\<F>\<^sub>\<L>,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
        using flt2goodTock_xs 2 flt2goodTock_consFL_imp
        by (metis True \<open>[[X]\<^sub>R] = flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> acceptance.distinct(1) acset ttobs.inject(2) flt2ttobs.simps(1) flt2goodTock_extend_consFL_last_e' fltrace_concat.simps(3) last_bullet_concatmix list.inject mem_Collect_eq)
      then show ?thesis
        by (metis (no_types, lifting) "2.prems"(2) Cons_eq_append_conv Finite_Linear_Model.butlast.simps(1) Finite_Linear_Model.last.simps(1) True \<open>[[X]\<^sub>R] = flt2ttobs \<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> \<open>\<rho> = flt2ttobs xs\<close> acceptance.distinct(1) acset ttobs.inject(2) fl_cons_acceptance_consFL flt2ttobs.simps(1) flt2ttobs_acceptance_cons_eq_list_cons flt2ttobs_last_tock flt2goodTock.simps(1) flt2goodTock_xs fltrace_concat2.simps(3) list.inject mem_Collect_eq)
    qed
  next
    case False
    then show ?thesis
...
next
  ...
  then show ?case
    proof (cases "last xs = \<bullet>")
      case True
      then show ?thesis
        by (metis "3.prems"(3) acceptance.distinct(1) last_cons_bullet_iff last_flt2ttobs_eq_ref_imp_last snoc_eq_iff_butlast)
   next
      case False
      then have flt2goodTock_xs:"flt2goodTock (xs)"
        using "3.prems"(5) concat_FL_last_not_bullet_absorb by force
      have "[[X]\<^sub>R] = flt2ttobs(\<langle>last xs\<rangle>\<^sub>\<F>\<^sub>\<L>)"
        using "3.prems"(3) False concat_FL_last_not_bullet_absorb flt2ttobs_last_fl_not_bullet_dist_list_cons flt2goodTock_xs by fastforce
      have "xs \<in> P"
        using 3 False
        by (metis concat_FL_last_not_bullet_absorb)
      then have "butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>last xs\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P"
      ...
      then have xs_e:"butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P"
        by (metis "3.prems"(2) CollectI FL2_def False List.last.simps \<open>[[X]\<^sub>R] = flt2ttobs \<langle>Finite_Linear_Model.last xs\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> assms(1) ttobs.inject(2) flt2ttobs.simps(1))
      then have "\<rho> @ [[X]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs(butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      proof -
        have f1: "flt2ttobs xs = \<rho> @ [[X]\<^sub>R]"
        ...
        have "Tock \<in>\<^sub>\<F>\<^sub>\<L> Finite_Linear_Model.last xs"
          using "3.prems"(2) False \<open>[[X]\<^sub>R] = flt2ttobs \<langle>Finite_Linear_Model.last xs\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> by force
        then show ?thesis
          using f2 f1 by (metis (no_types) List.last.simps append_butlast_last_id append_is_Nil_conv butlast_append fl_cons_acceptance_consFL flt2ttobs_last_tock flt2goodTock_xs last_appendR list.simps(3))
      qed
      then have "\<rho> @ [[X]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs(butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) \<and> butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P"
        using xs_e by auto
      then have "\<rho> @ [[X]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs(butlast xs &\<^sub>\<F>\<^sub>\<L> \<langle>(last xs,Tock)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) 
        ...
        using flt2goodTock_xs
        by (metis "3.prems"(2) CollectI False \<open>[[X]\<^sub>R] = flt2ttobs \<langle>Finite_Linear_Model.last xs\<rangle>\<^sub>\<F>\<^sub>\<L>\<close> ttobs.inject(2) fl_cons_acceptance_consFL flt2ttobs.simps(1) flt2goodTock_extend_consFL_last_fl_e list.inject)
      then show ?thesis by blast
    qed
qed
...
lemma TTM2a_fl2tt_for_FL2_FL1_FL0:
  assumes "FL2 P" "FL1 P" "FL0 P"
  shows "TTM2a(fl2tt P)"
proof -
  have "TTM2a(fl2tt P) = TTM2a({flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl} \<union> {[]})"
    using assms
    by (simp add: fl2tt_FL0_FL1_flt2goodTock)
  also have "... = TTM2a({flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl})"
    using TTM2a_union_empty_trace by auto
  also have "... = True"
    using assms unfolding fl2tt_def TTM2a_def apply auto
    using FL2_imp_TTM2a_part by blast
  finally show ?thesis by auto
qed

lemma TTM2b_fl2tt_for_FL2_FL1_FL0:
  assumes "FL2 P" "FL1 P" "FL0 P"
  shows "TTM2b(fl2tt P)"
proof -
  have "TTM2b(fl2tt P) = TTM2b({flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl} \<union> {[]})"
    using assms
    by (simp add: fl2tt_FL0_FL1_flt2goodTock)
  also have "... = TTM2b({flt2ttobs fl|fl. fl \<in> P \<and> flt2goodTock fl})"
    using TTM2b_union_empty_trace by auto
  also have "... = True"
    using assms unfolding fl2tt_def TTM2b_def apply auto
    using FL2_imp_TTM2b_part by blast
  finally show ?thesis by auto
qed

lemma Tick_of_Refuals_in_flt2ttobs:
  assumes "tickWF Tick fl" "\<rho> @ [[X]\<^sub>R] = flt2ttobs fl"
  shows "Tick \<in> X"
  using assms apply (induct fl arbitrary:X \<rho> rule:flt2ttobs.induct, auto)
   apply (case_tac A, auto)
...
lemma tickWF_add_Tick_refusal_trace_flt2ttobs_idem:
  ...
  shows "add_Tick_refusal_trace (flt2ttobs xs) = (flt2ttobs xs)"
  using assms apply(induct xs rule:flt2ttobs.induct, auto)
   apply (case_tac A, auto, case_tac a, auto, case_tac b, auto)
  by (case_tac A, auto, case_tac b, auto)
...
lemma TT3_trace_fl2ttobs:
  ...
  apply (induct fl rule:flt2ttobs.induct) apply auto[1]
...
next
  ...
  proof (cases "last xs = \<bullet>")
    case True
    then have \<rho>_X:"\<rho> @ [[X]\<^sub>R] = flt2ttobs (xs) @ flt2ttobs(\<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      using 2
      by (metis (no_types, lifting) Finite_Linear_Model.last.simps(1) append.right_neutral bullet_right_zero2 flt2ttobs.simps(1) flt2ttobs_last_fl_not_bullet_dist_list_cons last_bullet_butlast_last last_bullet_then_last_cons)
    then show ?thesis
      proof (cases x)
        case acnil
      ...
      next
        case (acset x2)
        then have "[[X]\<^sub>R] = flt2ttobs(\<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
          using \<rho>_X by auto
        then have X_x2:"X = {x. x \<notin> x2}"
          using acset by auto
        
        have a:"\<forall>e. (e \<notin> X \<and> e \<noteq> Tock) \<longrightarrow> (\<exists>fl. \<rho> @ [[e]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)"
          using "2.prems"(5) "2.prems"(6) "2.prems"(7) FL2_imp_TTM2a_part assms(1) by blast
        then have a2:"\<forall>e. (e \<in> x2 \<and> e \<noteq> Tock) \<longrightarrow> (\<exists>fl. \<rho> @ [[e]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)"
          using X_x2 by blast
        
        have b:"\<forall>e. (e \<notin> X \<and> e = Tock) \<longrightarrow> (\<exists>fl. \<rho> @ [[X]\<^sub>R, [Tock]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)"
          using "2.prems"(5) "2.prems"(6) "2.prems"(7) FL2_imp_TTM2b_part assms(1) by blast
        then have b2:"\<forall>e. (e \<in> x2 \<and> e = Tock) \<longrightarrow> (\<exists>fl. \<rho> @ [[X]\<^sub>R, [Tock]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)"
          using X_x2 by blast

        have "x2 \<subseteq> {e. e \<noteq> Tock \<and> (\<exists>fl. \<rho> @ [[e]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl) \<or> e = Tock \<and> (\<exists>fl. \<rho> @ [[X]\<^sub>R, [e]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)}"
          using a2 b2 by auto
  ...
  next
    case False
    then have "\<rho> @ [[X]\<^sub>R] = flt2ttobs (xs)"
              "xs \<in> P"
              "flt2goodTock xs"
...
next
  ...
  proof (cases "last xs = \<bullet>")
    ...
    then show ?thesis using 3
      by (metis (mono_tags, lifting) Nil_is_append_conv acceptance.distinct(1) last_cons_bullet_iff last_flt2ttobs_eq_ref_imp_last last_snoc not_Cons_self2)
  next
    case False
    then have "\<rho> @ [[X]\<^sub>R] = flt2ttobs (xs)"
...
next
  ...
  proof (cases "last xs = \<bullet>")
    case True
    then have \<rho>_X:"\<rho> @ [[X]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs (xs) @ flt2ttobs(\<langle>x\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      using 2
      by (metis (no_types, lifting) Finite_Linear_Model.last.simps(1) append.right_neutral bullet_right_zero2 flt2ttobs.simps(1) flt2ttobs_last_fl_not_bullet_dist_list_cons last_bullet_butlast_last last_bullet_then_last_cons)
    then show ?thesis
      proof (cases x)
    ...
    next
    case False
    then have "\<rho> @ [[X]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs (xs)"
              "xs \<in> P"
              "flt2goodTock xs"
      using 2 by (metis concat_FL_last_not_bullet_absorb)+
    then show ?thesis using 2
...
next
  ...
  proof (cases "last xs = \<bullet>")
    case True
    then have \<rho>_X:"\<rho> @ [[X]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs (xs) @ flt2ttobs(\<langle>x,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      using 3 
      by (metis (no_types, lifting) flt2ttobs_acceptance_cons_eq_list_cons flt2ttobs_not_flt2goodTock_imp_flt2ttobs_eq_consFL_any fltrace_concat2_assoc last_cons_bullet_iff)
    then have "flt2goodTock \<langle>x,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>"
      using "3.prems"(7) flt2goodTock_consFL_imp_rhs True by blast
    then have X_Tock:"[[X]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs(\<langle>x,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
    ...
    then have "[[X]\<^sub>R] = flt2ttobs(\<langle>[xA]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      using X_Tock by auto
    then have "\<rho> @ [[X]\<^sub>R] = flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>[xA]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
      by (metis (mono_tags, lifting) "3.prems"(7) Finite_Linear_Model.last.simps(1) True X_Tock \<rho>_X append_same_eq butlast_last_FL flt2ttobs_last_fl_not_bullet_dist_list_cons flt2goodTock_consFL_imp_lhs flt2goodTock_extend_consFL_acceptance last_bullet_butlast_last last_bullet_then_last_cons last_cons_acceptance_not_bullet)
    ...
    have a:"\<forall>e. (e \<notin> X \<and> e \<noteq> Tock) \<longrightarrow> (\<exists>fl. \<rho> @ [[e]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)"
      ...
      by (metis "3.prems"(7) Finite_Linear_Model.last.simps(1) True \<open>\<rho> @ [[X]\<^sub>R] = flt2ttobs (xs &\<^sub>\<F>\<^sub>\<L> \<langle>[xA]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L>)\<close> \<open>xs &\<^sub>\<F>\<^sub>\<L> \<langle>[xA]\<^sub>\<F>\<^sub>\<L>\<rangle>\<^sub>\<F>\<^sub>\<L> \<in> P\<close> assms(1) butlast_last_FL flt2goodTock_consFL_imp_lhs flt2goodTock_extend_consFL_acceptance last_bullet_butlast_last)
    then have a2:"\<forall>e. (e \<in> xA \<and> e \<noteq> Tock) \<longrightarrow> (\<exists>fl. \<rho> @ [[e]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)"
      using xA by blast

    have b:"\<forall>e. (e \<notin> X \<and> e = Tock) \<longrightarrow> (\<exists>fl. \<rho> @ [[X]\<^sub>R, [Tock]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)"
      using xA FL2_imp_TTM2b_part
      using "3.prems"(5) "3.prems"(6) "3.prems"(7) by blast
    then have b2:"\<forall>e. (e \<in> xA \<and> e = Tock) \<longrightarrow> (\<exists>fl. \<rho> @ [[X]\<^sub>R, [Tock]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)"
      using xA by blast

    have "xA \<subseteq> {e. e \<noteq> Tock \<and> (\<exists>fl. \<rho> @ [[e]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl) \<or> e = Tock \<and> (\<exists>fl. \<rho> @ [[X]\<^sub>R, [e]\<^sub>E] = flt2ttobs fl \<and> fl \<in> P \<and> flt2goodTock fl)}"
      using a2 b2 by auto
    then have "Y \<subseteq> X"
      using 3 xA by blast
    then have "X \<union> Y = X"
...
lemma flt2ttobs_split_cons:
  assumes "ys @ xs = flt2ttobs fl" "flt2goodTock fl" "ttWF ys" "ttWF xs"
  shows "\<exists>fl\<^sub>1 fl\<^sub>0. ys = flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 \<and> fl\<^sub>0 &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = fl \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1"
  using assms
proof (induct fl arbitrary:xs ys rule:flt2ttobs.induct)
  case (1 A)
  then show ?case 
    apply (cases A, auto)
...
next
  ...
  then have flt2ttobs_A_fl:"flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl) = flt2ttobs ((eA, e)\<^sub>\<F>\<^sub>\<L> #\<^sub>\<F>\<^sub>\<L> fl)"
      using 2 by auto
  ...
  proof (induct ys)
    case Nil
    then show ?case
      by (metis append_Nil bullet_left_zero2 flt2ttobs.simps(1) flt2goodTock.simps(1))
  next
    case (Cons a ys)
    then show ?case
    proof (cases a)
      ...
      proof (cases x1)
        case (Event e1)
        then have ys_xs:"([Event e1]\<^sub>E # ys) @ xs = flt2ttobs ((eA, e)\<^sub>\<F>\<^sub>\<L> #\<^sub>\<F>\<^sub>\<L> fl)"
          using ObsEvent Cons flt2ttobs_A_fl by auto
        then have "e = Event e1"
          using eA by (cases e, auto, cases eA, auto)
        then have e1_flt2ttobs:"[[Event e1]\<^sub>E] = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
                  "ys @ xs = flt2ttobs fl"
          using eA ys_xs by auto
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. ys = flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 \<and> fl\<^sub>0 &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = fl \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1"
          using ObsEvent Cons
          by (metis Event ttWF.simps(4) flt2goodTock.simps(2))
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. [Event e1]\<^sub>E # ys = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 
                                \<and> fl\<^sub>0 &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = fl \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1" 
          using e1_flt2ttobs by auto
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. [Event e1]\<^sub>E # ys = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 
          ...
          by auto
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. [Event e1]\<^sub>E # ys = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) \<and> xs = flt2ttobs fl\<^sub>1 
        ...
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. a # ys = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) \<and> xs = flt2ttobs fl\<^sub>1 
      ...
      next
        case Tick
        then have ys_xs:"([Tick]\<^sub>E # ys) @ xs = flt2ttobs ((eA, e)\<^sub>\<F>\<^sub>\<L> #\<^sub>\<F>\<^sub>\<L> fl)"
          using ObsEvent Cons flt2ttobs_A_fl by auto
        then have "e = Tick"
          using eA by (cases e, auto, cases eA, auto)
        then have e1_flt2ttobs:"[[Tick]\<^sub>E] = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
                  "ys @ xs = flt2ttobs fl"
          using eA ys_xs by auto
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. ys = flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 \<and> fl\<^sub>0 &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = fl \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1"
          using ObsEvent Cons
          by (metis Tick append_Nil bullet_left_zero2 ttWF.simps(8) flt2ttobs.simps(1) flt2goodTock.simps(1) flt2goodTock.simps(2) neq_Nil_conv)
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. [Tick]\<^sub>E # ys = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 
                                \<and> fl\<^sub>0 &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = fl \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1" 
          using e1_flt2ttobs by auto
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. [Tick]\<^sub>E # ys = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 
                                \<and> (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = (eA, e)\<^sub>\<F>\<^sub>\<L> #\<^sub>\<F>\<^sub>\<L> fl \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1" 
          by auto
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. [Tick]\<^sub>E # ys = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) \<and> xs = flt2ttobs fl\<^sub>1 
                                \<and> (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = (eA, e)\<^sub>\<F>\<^sub>\<L> #\<^sub>\<F>\<^sub>\<L> fl \<and> flt2goodTock (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) \<and> flt2goodTock fl\<^sub>1" 
      ...
      then show ?thesis using Cons
        proof (induct ys)
          ...
          proof (induct xs)
            case Nil
            then show ?case
              by (metis append.right_neutral bullet_right_zero2 flt2ttobs.simps(1) flt2goodTock.simps(1))
          next
            case Cons_xs:(Cons b zs)
            then have "[a] @ b # zs = flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl)"
              using Cons_xs Cons
              by blast
        ...
        next
          ...
          then have "([x2]\<^sub>R # [Tock]\<^sub>E # zs) @ xs = flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl)"
            using Cons_ys.prems(5) Ref by blast 
          then have ys_xs:"([x2]\<^sub>R # [Tock]\<^sub>E # zs) @ xs = flt2ttobs ((eA, e)\<^sub>\<F>\<^sub>\<L> #\<^sub>\<F>\<^sub>\<L> fl)"
            using Cons flt2ttobs_A_fl b_Tock
            by presburger
          then have "[[x2]\<^sub>R,[Tock]\<^sub>E] @ (zs @ xs) = flt2ttobs ((eA, e)\<^sub>\<F>\<^sub>\<L> #\<^sub>\<F>\<^sub>\<L> fl)"
            by auto
          then have "[[x2]\<^sub>R,[Tock]\<^sub>E] @ (zs @ xs) = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl)"
            by auto
          then have e1_flt2ttobs:"[[x2]\<^sub>R,[Tock]\<^sub>E] = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
                  "zs @ xs = flt2ttobs fl"
          ...
          then have flt2goodTock_eA: "flt2goodTock (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>)"
            by auto
          then have "\<exists>fl\<^sub>1 fl\<^sub>0. zs = flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 \<and> fl\<^sub>0 &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = fl \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1"
            using Cons e1_flt2ttobs
            by (metis Cons_ys.prems(7) Ref b_Tock ttWF.simps(5) flt2goodTock.simps(2))
          then have "\<exists>fl\<^sub>1 fl\<^sub>0. [x2]\<^sub>R # [Tock]\<^sub>E # zs = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 
                                \<and> fl\<^sub>0 &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = fl \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1" 
          using e1_flt2ttobs
        ...
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. [x2]\<^sub>R # [Tock]\<^sub>E # zs = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L>) @ flt2ttobs fl\<^sub>0 \<and> xs = flt2ttobs fl\<^sub>1 
                                \<and> (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = (eA, e)\<^sub>\<F>\<^sub>\<L> #\<^sub>\<F>\<^sub>\<L> fl \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1" 
          by auto
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. [x2]\<^sub>R # [Tock]\<^sub>E # zs = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) \<and> xs = flt2ttobs fl\<^sub>1 
                                \<and> (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = (eA, e)\<^sub>\<F>\<^sub>\<L> #\<^sub>\<F>\<^sub>\<L> fl \<and> flt2goodTock (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) \<and> flt2goodTock fl\<^sub>1" 
          using eA flt2goodTock_eA 
          apply auto
            apply blast
          by (cases e, auto)+
        then have "\<exists>fl\<^sub>1 fl\<^sub>0. a # b # zs = flt2ttobs (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) \<and> xs = flt2ttobs fl\<^sub>1 
                                \<and> (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = A #\<^sub>\<F>\<^sub>\<L> fl \<and> flt2goodTock (\<langle>(eA, e)\<^sub>\<F>\<^sub>\<L>,\<bullet>\<rangle>\<^sub>\<F>\<^sub>\<L> &\<^sub>\<F>\<^sub>\<L> fl\<^sub>0) \<and> flt2goodTock fl\<^sub>1" 
...
next
  ...
  proof -
    have "ttWF(\<rho> @ [X]\<^sub>R # a # list)"
      by (metis Cons_eq_append_conv FLTick0_def assms(4) assms(6) assms(7) ttWF_dist_cons_refusal' flt2ttobs_is_ttWF local.Cons)
  ...
  then obtain fl\<^sub>0 fl\<^sub>1 where fls:"\<rho> @ [[X]\<^sub>R,a] = flt2ttobs fl\<^sub>0 \<and> flt2goodTock fl\<^sub>0 \<and> flt2goodTock fl\<^sub>1 \<and> list = flt2ttobs fl\<^sub>1 \<and> fl\<^sub>0 &\<^sub>\<F>\<^sub>\<L> fl\<^sub>1 = fl"
    using assms flt2ttobs_split_cons ttWF_cons ttWF_list 
...
lemma TT1c_fl2tt_part:
  ...
  using assms  
proof (induct fl arbitrary:\<rho> rule:flt2ttobs.induct)
case (1 A)
  then show ?case 
    apply (cases A, auto)
...
next
  ...
  proof (cases "event A = Tock \<and> acceptance A \<noteq> \<bullet>")
    ...
    then have "A = (acceptance(A),Tock)\<^sub>\<F>\<^sub>\<L> \<and> Tock \<in>\<^sub>\<F>\<^sub>\<L> acceptance(A)"
      by (metis Rep_aevent_inverse acceptance.rep_eq event.rep_eq event_in_acceptance prod.collapse)
    then have flt2ttobs_A_fl:"flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl) = [{x. x \<notin>\<^sub>\<F>\<^sub>\<L> acceptance(A)}]\<^sub>R # [Tock]\<^sub>E # flt2ttobs fl"
      using True by auto
    then have "\<rho> \<le>\<^sub>C flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl) = (\<rho> \<le>\<^sub>C ([{x. x \<notin>\<^sub>\<F>\<^sub>\<L> acceptance(A)}]\<^sub>R # [Tock]\<^sub>E # flt2ttobs fl))"
      using True by auto
    ...
    next
      ...
      next
        ...
        then have "z = [Tock]\<^sub>E"
          by auto
        then have "a # z # zs \<le>\<^sub>C flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl)"
          using Cons by blast
        then have "[{x. x \<notin>\<^sub>\<F>\<^sub>\<L> acceptance(A)}]\<^sub>R # [Tock]\<^sub>E # zs \<le>\<^sub>C flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl)"
          using Cons.prems(1) \<open>z = [Tock]\<^sub>E\<close> by blast
        then have "[{x. x \<notin>\<^sub>\<F>\<^sub>\<L> acceptance(A)}]\<^sub>R # [Tock]\<^sub>E # zs \<le>\<^sub>C [{x. x \<notin>\<^sub>\<F>\<^sub>\<L> acceptance(A)}]\<^sub>R # [Tock]\<^sub>E # flt2ttobs fl"
          using flt2ttobs_A_fl by auto
        then have "zs \<le>\<^sub>C flt2ttobs fl"
          by auto
        then have "\<exists>fl\<^sub>2. zs = flt2ttobs fl\<^sub>2 \<and> fl\<^sub>2 \<le> fl"
          using Cons True by blast
        then have "\<exists>fl\<^sub>2. [{x. x \<notin>\<^sub>\<F>\<^sub>\<L> acceptance(A)}]\<^sub>R # [Tock]\<^sub>E # zs = [{x. x \<notin>\<^sub>\<F>\<^sub>\<L> acceptance(A)}]\<^sub>R # [Tock]\<^sub>E # flt2ttobs fl\<^sub>2 \<and> A #\<^sub>\<F>\<^sub>\<L> fl\<^sub>2 \<le> A #\<^sub>\<F>\<^sub>\<L> fl"
          by auto
        then have "\<exists>fl\<^sub>2. [{x. x \<notin>\<^sub>\<F>\<^sub>\<L> acceptance(A)}]\<^sub>R # [Tock]\<^sub>E # zs = flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl\<^sub>2) \<and> A #\<^sub>\<F>\<^sub>\<L> fl\<^sub>2 \<le> A #\<^sub>\<F>\<^sub>\<L> fl"
          using True by auto
        then have "\<exists>fl\<^sub>2. a # z # zs = flt2ttobs (A #\<^sub>\<F>\<^sub>\<L> fl\<^sub>2) \<and> A #\<^sub>\<F>\<^sub>\<L> fl\<^sub>2 \<le> A #\<^sub>\<F>\<^sub>\<L> fl"
          using Cons by auto
        then have "\<exists>fl\<^sub>2. a # z # zs = flt2ttobs fl\<^sub>2 \<and> fl\<^sub>2 \<le> A #\<^sub>\<F>\<^sub>\<L> fl"
          by blast
        then show ?case by blast
      qed
    qed
  next
    ...
    then show ?thesis using 2 apply auto
      apply (metis (no_types, hide_lams) bullet_left_zero2 tt_prefix.simps(2) flt2ttobs.simps(1) flt2ttobs.simps(2) less_eq_fltrace.simps(3) neq_Nil_conv order_refl x_le_x_concat2)  
      apply (cases A, auto)
      apply (case_tac b, auto)
      apply (metis (no_types, hide_lams) acceptance_event bullet_left_zero2 tt_prefix.simps(2) ttevent.distinct(1) flt2ttobs.simps(1) flt2ttobs.simps(2) less_eq_fltrace.simps(3) neq_Nil_conv order_refl x_le_x_concat2)
      using tt_prefix_split apply force
      by (metis (no_types, hide_lams) acceptance_event bullet_left_zero2 tt_prefix.simps(2) ttevent.distinct(5) flt2ttobs.simps(1) flt2ttobs.simps(2) less_eq_fltrace.simps(3) neq_Nil_conv order_refl x_le_x_concat2)
  qed
qed
